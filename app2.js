...
function showResponse(r) {
  ...
  // Download button for binary/image
  ...
  if (r._isBinary && r._dataUrl) {
    ...
    if (ct2.indexOf('image/') === 0) {
      writeIframe(iframeEl,
        '<html><body style="margin:0;background:#1a1a2e;display:flex;align-items:center;justify-content:center;min-height:100vh">' +
        '<img src="' + r._dataUrl + '" style="max-width:100%;max-height:100vh;object-fit:contain"></body></html>');
    } else {
      writeIframe(iframeEl,
        '<html><body style="font-family:sans-serif;padding:30px;color:#888;background:#111;text-align:center">' +
        '<div style="font-size:48px">📦</div><p>Binary file — use ⬇ Download to save.</p>' +
        '<p style="font-size:12px">Type: <code>' + esc(ct2) + '</code> &nbsp; Size: ' + formatBytes(r._size) + '</p></body></html>');
    }
    ...
    writeIframe(iframeEl,
      ...
      '<p style="font-size:12px">Use <strong>Pretty</strong> or <strong>Raw</strong> tab to view.</p></body></html>');
  }
...
function copyResponse() {
  var text = document.getElementById('resp-raw').textContent;
  ...
  c.innerHTML =
    ...
    _testResults.map(function(t){
      return '<div class="tr-item ' + (t.pass?'tr-pass':'tr-fail') + '">' +
        ...
        '<div><div class="tr-name">' + esc(t.name) + '</div>' +
...
function openEnlargeResp() {
  ...
  var toolbar = document.getElementById('fs-toolbar');
...
function fsSwitchView(view) {
  _fsCurrentView = view;
  document.querySelectorAll('.fs-tab').forEach(function(b){
...
function openEnlargePanel(panel) {
  ...
  if (panel === 'headers') {
    ...
    table.innerHTML = Object.keys((_lastResponse&&_lastResponse._headers)||{}).map(function(k){
      return '<tr><td>' + esc(k) + '</td><td>' + esc(_lastResponse._headers[k]) + '</td></tr>';
    }).join('') || '<tr><td colspan="2" style="color:var(--text3);padding:14px">No headers</td></tr>';
    ...
    var clone = document.getElementById('cookies-out').cloneNode(true);
    clone.style.padding = '14px'; fsBody.innerHTML = ''; fsBody.appendChild(clone);
  } else if (panel === 'tests') {
    title.textContent = 'Test Results'; copyBtn.style.display = 'none'; saveBtn.style.display = 'none';
    var clone2 = document.getElementById('test-output').cloneNode(true);
    clone2.style.padding = '14px'; fsBody.innerHTML = ''; fsBody.appendChild(clone2);
  } else if (panel === 'console') {
    title.textContent = 'Console'; copyBtn.style.display = 'none'; saveBtn.style.display = 'none';
    var clone3 = document.getElementById('console-out').cloneNode(true);
    clone3.style.padding = '0'; fsBody.innerHTML = ''; fsBody.appendChild(clone3);
  }
  overlay.style.display = 'flex';
}
...
function fsAction(action) {
  if (action === 'copy') {
    var text = '';
    if (_fsPanel === 'body')    text = (_lastResponse && _lastResponse._body) || '';
    else if (_fsPanel === 'headers') text = Object.keys((_lastResponse&&_lastResponse._headers)||{}).map(function(k){ return k + ': ' + _lastResponse._headers[k]; }).join('\n');
    navigator.clipboard.writeText(text).then(function(){ notify('Copied!','success'); });
  } else if (action === 'save') {
    if (_lastResponse && _lastResponse._isBinary && _lastResponse._dataUrl) { downloadBinaryResp(); return; }
...
// ─────────────────────────────────────────────────────────────
// HISTORY — full replay of all 8 sections
// ─────────────────────────────────────────────────────────────
...
function renderHistory() {
  ...
  function row(h) {
    var p = h.pinned === true;
    // Build body preview snippet
    var bp = '';
    if      (h.bodyType==='raw'    && h.rawBody)         bp = h.rawBody.slice(0,55);
    else if (h.bodyType==='urlenc' && h.urlEncoded&&h.urlEncoded.length) bp = h.urlEncoded.map(function(r){return r.k+'='+r.v;}).join('&').slice(0,55);
    else if (h.bodyType==='form'   && h.formFields&&h.formFields.length) bp = h.formFields.map(function(r){return r.k;}).join(', ').slice(0,55);
    else if (h.bodyType==='graphql'&& h.gqlQ)             bp = h.gqlQ.slice(0,55);
    ...
    return '<div class="hist-row' + (p?' pinned':'') + '" data-hid="' + h.id + '">' +
      ...
      '<div class="hist-main">' +
        ...
        (bp ? '<span class="hist-body-preview" title="' + esc(bp) + '">' + esc(bp.length>=55?bp+'…':bp) + '</span>' : '') +
      '</div>' +
...
function unpinAllHistory() {
  var n = S.history.filter(function(h){ return h.pinned===true; }).length;
  if (!n) { notify('Nothing is pinned','info'); return; }
  S.history.forEach(function(h){ h.pinned=false; });
...
async function runAdvRepeat() {
  ...
  var count   = Math.max(1, Math.min(500, parseInt(document.getElementById('adv-count').value)||5));
  var delay   = Math.max(0, parseInt(document.getElementById('adv-delay').value)||0);
  var resultsEl = document.getElementById('adv-results');
  var pbWrap    = document.getElementById('adv-pw');
  var pb        = document.getElementById('adv-pb');
  var pt        = document.getElementById('adv-pt');
  var runBtn    = document.getElementById('adv-run-btn');
  ...
  for (var i = 0; i < count; i++) {
    ...
    var num = i + 1;
    try {
      var url = resolveVars(h.url||'');
      var fH  = {};
      if(h.headers) h.headers.filter(function(x){return x.on!==false&&x.k;}).forEach(function(x){ fH[x.k]=x.v; });
      var resp = await fetchDirect(url, h.method||'GET', fH, h.bodyType==='raw'&&h.rawBody?h.rawBody:null);
      var ok = resp.status>=200&&resp.status<300;
      if(ok) passed++; else failed++;
      var rowEl = document.createElement('div'); rowEl.className='adv-result-row';
      rowEl.innerHTML='<span class="adv-result-num">#'+num+'</span>'+
      ...
      failed++;
      var rowEl2=document.createElement('div'); rowEl2.className='adv-result-row';
...
function renderCollections() {
  var q=document.getElementById('coll-search').value.toLowerCase(), list=document.getElementById('coll-list');
  var filtered=S.collections.filter(function(c){return c.name.toLowerCase().indexOf(q)!==-1;});
  if(!filtered.length){list.innerHTML='<div class="empty-state"><div class="ei">📂</div><p>No collections yet.</p></div>';return;}
  list.innerHTML=filtered.map(function(c){return renderCollItem(c);}).join('');
}
function renderCollItem(c) {
  var items=(c.requests||[]).map(function(item){
    return item._isFolder
      ? '<div class="coll-folder"><div class="folder-header" onclick="toggleFolder(\''+c.id+'\',\''+item.id+'\')"><span class="folder-arrow" id="fa-'+item.id+'">▶</span>📁 '+esc(item.name)+'</div><div class="folder-reqs" id="fr-'+item.id+'">'+(item.requests||[]).map(function(r){return reqRowHtml(c.id,r,true);}).join('')+'</div></div>'
...
function reqRowHtml(collId,r,inFolder) {
...
function runCollModal(e,id){
  e.stopPropagation();
  var coll=S.collections.find(function(c){return c.id===id;});
  if(!coll||!coll.requests||!coll.requests.length){notify('Collection is empty','error');return;}
  var reqs=(coll.requests||[]).filter(function(r){return !r._isFolder;});
  openModal('<div class="modal-bg"><div class="modal lg"><div class="mh"><span class="mh-title">▶ Runner — '+esc(coll.name)+'</span><button class="m-close" onclick="closeModal()">✕</button></div><div class="mb">'+
    ...
    '<div style="margin-bottom:10px"><div class="field-label">SELECT REQUESTS</div>'+
      reqs.map(function(r){return '<div class="cr-req-item"><input type="checkbox" class="cr-req-chk kv-chk" data-rid="'+r.id+'" checked><span class="mbadge '+r.method+'" style="color:'+(MC[r.method]||'var(--text2)')+'">'+r.method+'</span><span>'+esc(r.name)+'</span></div>';}).join('')+
    '</div>'+
...
async function doRunColl(id){
  var coll=S.collections.find(function(c){return c.id===id;});if(!coll)return;
  var iters=parseInt(document.getElementById('cr-iter').value)||1,delay=parseInt(document.getElementById('cr-delay').value)||0,stop=document.getElementById('cr-stop').checked;
  var checkedIds=new Set(Array.from(document.querySelectorAll('.cr-req-chk:checked')).map(function(el){return el.dataset.rid;}));
  var reqs=(coll.requests||[]).filter(function(r){return !r._isFolder&&checkedIds.has(r.id);});
  if(!reqs.length){notify('No requests selected','error');return;}
  var dataRows=[{}];
  var dataFile=document.getElementById('cr-data')&&document.getElementById('cr-data').files&&document.getElementById('cr-data').files[0];
  if(dataFile){try{var txt=await dataFile.text();if(dataFile.name.endsWith('.json')){dataRows=JSON.parse(txt);if(!Array.isArray(dataRows))dataRows=[dataRows];}else{var lines=txt.trim().split('\n'),hdrs=lines[0].split(',').map(function(h){return h.trim().replace(/^"|"$/g,'');});dataRows=lines.slice(1).map(function(line){var vals=line.split(',').map(function(v){return v.trim().replace(/^"|"$/g,'');});return Object.fromEntries(hdrs.map(function(h,i){return[h,vals[i]||''];}));});}}catch(e){notify('Data file error: '+e.message,'error');return;}}
  var total=iters*dataRows.length*reqs.length,done=0,passed=0,failed=0;
  var resultEl=document.getElementById('cr-results');resultEl.innerHTML='';
  for(var iter=0;iter<iters;iter++){for(var di=0;di<dataRows.length;di++){_iterInfo={iteration:iter,iterationCount:iters,dataRow:dataRows[di]};for(var ri=0;ri<reqs.length;ri++){var req=reqs[ri];
    try{
      var url=resolveVars(req.url||'',dataRows[di]);var h2={};
      (req.headers||[]).filter(function(x){return x.on!==false&&(x.k||x.key);}).forEach(function(x){h2[resolveVars(x.k||x.key,dataRows[di])]=resolveVars(x.v||x.value||'',dataRows[di]);});
      if(req.preScript&&req.preScript.trim()){var pm2=buildPM(null,coll.variables||{});runScript(req.preScript,pm2);}
      var ro=await fetchDirect(url,req.method||'GET',h2,req.rawBody&&['GET','HEAD'].indexOf(req.method)===-1?resolveVars(req.rawBody,dataRows[di]):null);
      _testResults=[];if(req.testScript&&req.testScript.trim()){var pm3=buildPM(ro,coll.variables||{});runScript(req.testScript,pm3);}
      var pt2=_testResults.filter(function(t){return t.pass;}).length,isOk=ro.status>=200&&ro.status<300&&(!_testResults.length||pt2===_testResults.length);
      if(isOk)passed++;else failed++;
      resultEl.innerHTML+='<div class="cr-result-item '+(isOk?'pass':'fail')+'"><span>'+(isOk?'✅':'❌')+'</span><div><div style="font-weight:600;font-size:12px">['+( iter+1)+'/'+iters+'] '+esc(req.name)+' — '+ro.status+' '+ro.statusText+'</div>'+_testResults.map(function(t){return '<div style="font-size:11px;color:'+(t.pass?'var(--ok)':'var(--err)')+';margin-top:2px">'+(t.pass?'✓':'✗')+' '+esc(t.name)+(t.error?' — '+esc(t.error):'')+'</div>';}).join('')+'</div></div>';
    ...
    done++;var bar=document.getElementById('cr-bar');if(bar)bar.style.width=Math.round(done/total*100)+'%';resultEl.scrollTop=resultEl.scrollHeight;if(delay>0)await sleep(delay);
  }}}
...
function addToColl(e,id){e.stopPropagation();saveTabUI();var tab=getActiveTab(),coll=S.collections.find(function(c){return c.id===id;});if(!coll||!tab)return;var name=prompt('Request name:',tab.name||'New Request');if(!name)return;if(!coll.requests)coll.requests=[];coll.requests.push({id:uid(),name:name,method:tab.method||'GET',url:tab.url||'',headers:tab.headers||[],params:tab.params||[],rawBody:tab.rawBody||'',bodyType:tab.bodyType||'none',rawFmt:tab.rawFmt||'json',authType:tab.authType||'none',authData:tab.authData||{},preScript:tab.preScript||'',testScript:tab.testScript||''});save();renderCollections();notify('Saved to collection!','success');}
...
function genCode(lang){
  ...
  if(out) out.value=(codes[lang]||(function(){return codes['cURL']();}))();
}
...
function doImport(){
  ...
  try{
    ...
    if(data.values&&(data.name||data.id)){var env={id:uid(),name:data.name||'Imported Env',variables:{}};(data.values||[]).forEach(function(v){env.variables[v.key]=v.value;});S.envs.push(env);save();renderEnvs();closeModal();notify('✅ Env "'+env.name+'" imported','success');return;}
    if(Array.isArray(data)&&data[0]&&data[0].requests){S.collections.push.apply(S.collections,data);save();renderCollections();closeModal();notify('Imported '+data.length+' collections','success');return;}
...
function importCurl(curl){
  try{
    var mm=curl.match(/-X\s+(\w+)/i)||curl.match(/--request\s+(\w+)/i);
    var um=curl.match(/curl\s+(?:-[^\s]+\s+)*['"]?([^\s'"]+)['"]?/);
    var hm=Array.from(curl.matchAll(/-H\s+['"]([^'"]+)['"]/gi));
    var dm=curl.match(/(?:--data(?:-raw|-binary)?|-d)\s+['"]([^'"]*)['"]/i)||curl.match(/--data '([^']*)'/i);
    var method=((mm&&mm[1])||'GET').toUpperCase(), url=(um&&um[1])||'';
    var headers=hm.map(function(m){var parts=m[1].split(':');var k=parts.shift();return{id:uid(),on:true,k:k.trim(),v:parts.join(':').trim(),desc:''};});
    var body=(dm&&dm[1])||'';
    newTab({method:method,url:url,name:url.replace(/^https?:\/\//,'').slice(0,40)||'Imported',headers:headers,rawBody:body,bodyType:body?'raw':'none',rawFmt:'json'});
    notify('Imported from cURL!','success');
  }catch(e){notify('cURL parse error: '+e.message,'error');}
}
function importOpenAPI(spec){
  var coll={id:uid(),name:(spec.info&&spec.info.title)||'OpenAPI Import',desc:(spec.info&&spec.info.description)||'',requests:[],variables:{}};
  var base=((spec.servers&&spec.servers[0]&&spec.servers[0].url)||'')+(spec.basePath||'');
  Object.keys(spec.paths||{}).forEach(function(path){
    ['get','post','put','patch','delete','head','options'].forEach(function(m){
      var pathItem=spec.paths[path]; if(!pathItem[m])return; var op=pathItem[m];
      var hdrs=[],params=[];
      (op.parameters||[]).forEach(function(p){if(p.in==='header')hdrs.push({id:uid(),on:true,k:p.name,v:p.example||'',desc:p.description||''});else if(p.in==='query')params.push({id:uid(),on:true,k:p.name,v:p.example||'',desc:p.description||''}); });
      var rawBody='',bodyType='none';
      if(op.requestBody){var ct=op.requestBody.content||{};var j=ct['application/json'];if(j&&j.example){rawBody=JSON.stringify(j.example,null,2);bodyType='raw';}}
      coll.requests.push({id:uid(),name:op.summary||op.operationId||(m.toUpperCase()+' '+path),method:m.toUpperCase(),url:base+path,headers:hdrs,params:params,rawBody:rawBody,bodyType:bodyType,rawFmt:'json',authType:'none',authData:{},preScript:'',testScript:''});
    });
  });
...
function openSettings(){
  ...
  openModal('<div class="modal-bg"><div class="modal lg"><div class="mh"><span class="mh-title">⚙ Settings</span><button class="m-close" onclick="closeModal()">✕</button></div><div class="mb">'+
    ...
    '<div class="s-sec"><div class="s-sec-title">ABOUT</div>'+
      '<p style="font-size:12px;color:var(--text3);line-height:1.8">PostmanWeb v4 — Full API Testing Platform.<br>All data stored locally. Worker: <span style="color:var(--accent)">square-credit-8186.donthulanithish53.workers.dev</span></p>'+
    '</div>'+
