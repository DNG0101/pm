...
// Fix any legacy history entries that have pinned=undefined/null → false
// This eliminates the "all requests look pinned" bug from old data
...
// 10.x, 192.168.x, 172.16-31.x, 127.x, localhost → DIRECT (no proxy)
// ─────────────────────────────────────────────────────────────
const PRIV = [
  /^localhost$/i,
  /^127\.\d+\.\d+\.\d+$/,
...
function refreshDirectBadge(urlStr) {
  ...
  if (!b) return;
  b.classList.toggle('visible', isPrivate(urlStr || ''));
}
...
async function hmacB64(algo, key, data) {
  const b = await _hmac(algo, key, data); return btoa(String.fromCharCode(...b));
}
...
function validateSchema(data, schema) {
  ...
  function chk(d,s,p){
    ...
    if('const'in s&&JSON.stringify(d)!==JSON.stringify(s.const))errors.push(`${p}: expected const ${JSON.stringify(s.const)}`);
    if(s.enum&&!s.enum.some(v=>JSON.stringify(v)===JSON.stringify(d)))errors.push(`${p}: not in enum`);
    if(typeof d==='string'){if(s.minLength!==undefined&&d.length<s.minLength)errors.push(`${p}: minLength ${s.minLength}`);if(s.maxLength!==undefined&&d.length>s.maxLength)errors.push(`${p}: maxLength ${s.maxLength}`);if(s.pattern&&!new RegExp(s.pattern).test(d))errors.push(`${p}: pattern failed`);if(s.format==='email'&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d))errors.push(`${p}: invalid email`);}
    if(typeof d==='number'){if(s.minimum!==undefined&&d<s.minimum)errors.push(`${p}: min ${s.minimum}`);if(s.maximum!==undefined&&d>s.maximum)errors.push(`${p}: max ${s.maximum}`);if(s.multipleOf!==undefined&&d%s.multipleOf!==0)errors.push(`${p}: multipleOf ${s.multipleOf}`);}
    if(Array.isArray(d)){if(s.minItems!==undefined&&d.length<s.minItems)errors.push(`${p}: minItems ${s.minItems}`);if(s.maxItems!==undefined&&d.length>s.maxItems)errors.push(`${p}: maxItems ${s.maxItems}`);if(s.items)d.forEach((x,i)=>chk(x,s.items,`${p}[${i}]`));}
    if(d!==null&&typeof d==='object'&&!Array.isArray(d)){(s.required||[]).forEach(k=>{if(!(k in d))errors.push(`${p}: missing '${k}'`);});if(s.properties)Object.entries(s.properties).forEach(([k,ps])=>{if(k in d)chk(d[k],ps,`${p}.${k}`);});if(s.additionalProperties===false&&s.properties)Object.keys(d).forEach(k=>{if(!s.properties[k])errors.push(`${p}: extra prop '${k}'`);});}
...
function buildPM(response, collVars={}) {
  ...
  const chai=(val)=>{
    const self={
      ...
      be:{
        ...
        at:{least:x=>{if(!(val>=x))throw new Error(`Expected ${val} >= ${x}`);return self;},most:x=>{if(!(val<=x))throw new Error(`Expected ${val} <= ${x}`);return self;}},
      ...
      have:{
        property:(p,v)=>{if(typeof val!=='object'||val===null||!(p in val))throw new Error(`Expected property "${p}"`);if(v!==undefined&&val[p]!==v)throw new Error(`Property "${p}" expected ${JSON.stringify(v)}, got ${JSON.stringify(val[p])}`);return self;},
        ...
        header:(key,value)=>{if(!response)throw new Error('No response');const hv=response._headers?.[key.toLowerCase()];if(!hv)throw new Error(`Missing header: ${key}`);if(value!==undefined&&hv!==String(value))throw new Error(`Header "${key}" expected "${value}", got "${hv}"`);return self;},
  ...
  const pmResp=response?{
    ...
    to:{
      ...
      be:{
        ok:         ()=>{if(response.status<200||response.status>=300)throw new Error(`Status ${response.status} not OK`);},
...
function newTab(d){
  const t=mkTab(d);S.tabs.push(t);S.activeId=t.id;
...
const AUTH_HTML={
  ...
  apikey:`<div class="af"><label>KEY NAME</label><input type="text" id="a-key" placeholder="e.g. X-API-Key"></div><div class="af"><label>KEY VALUE</label><input type="text" id="a-key-val" placeholder="your-api-key (supports {{variable}})"></div><div class="af"><label>ADD TO</label><select id="a-key-in"><option value="header">Header</option><option value="query">Query Params</option></select></div>`,
...
// ─────────────────────────────────────────────────────────────
// SEND REQUEST
// Private IPs → always DIRECT. Public URLs → proxy if corsEnabled.
// ─────────────────────────────────────────────────────────────
function cancelReq(){
  _abortCtrl?.abort();
...
async function sendRequest(){
  ...
  if(!disableBody&&!['GET','HEAD'].includes(method)){
    if(_bodyType==='raw'){
      ...
      if(!headers['Content-Type']&&!headers['content-type']){const ctMap={json:'application/json',xml:'application/xml',html:'text/html',text:'text/plain',javascript:'application/javascript'};headers['Content-Type']=ctMap[document.getElementById('raw-fmt').value]||'text/plain';}
    }else if(_bodyType==='urlenc'){
      const rows=readKV('urlenc').filter(r=>r.on&&r.k);body=rows.map(r=>`${encodeURIComponent(resolveVars(r.k))}=${encodeURIComponent(resolveVars(r.v))}`).join('&');headers['Content-Type']='application/x-www-form-urlencoded';
    }else if(_bodyType==='form'){
      const fd=new FormData();
      document.querySelectorAll('#kv-form tr').forEach(tr=>{const chk=tr.querySelector('.kv-chk'),key=tr.querySelectorAll('input[type=text]')[0]?.value,typ=tr.querySelector('.fv-type-sel')?.value||'text';if(!chk?.checked||!key)return;if(typ==='file'){const f=tr.querySelector('.fv-file input[type=file]')?.files?.[0];if(f)fd.append(key,f);}else fd.append(key,resolveVars(tr.querySelector('.fv-text input')?.value||''));});
      body=fd;
      ...
      const f=document.getElementById('bin-file')?.files?.[0];if(f){body=f;if(!headers['Content-Type'])headers['Content-Type']=f.type||'application/octet-stream';}
    }
  }
  ...
  if(mock){
    ...
    showResponse(fr);renderTests();flushConsole();notify('🎭 Mock '+fr.status+' returned','info');return;
  }

  // ── ROUTING: private IPs always direct, never through proxy ──
  ...
  try{
    ...
    if(e.name==='AbortError'){notify('Request cancelled / timed out','info');}
      ...
      const hint=isDirect
        ?`${e.message}\n\n💡 Private/internal IP called directly. Ensure the server is reachable from your browser (same network).`
...
function showResponse(r){
  const pill=document.getElementById('r-pill'),rtime=document.getElementById('r-time'),rsize=document.getElementById('r-size'),hint=document.getElementById('r-hint'),acts=document.getElementById('r-acts');
  if(!r){[pill,rtime,rsize,acts].forEach(el=>{if(el)el.style.display='none';});if(hint)hint.style.display='';document.getElementById('resp-pretty').innerHTML='';document.getElementById('resp-raw').textContent='';return;}
  pill.style.display='';pill.textContent=`${r.status} ${r.statusText}`;pill.className=`spill ${r._mock?'smock':'s'+Math.floor(r.status/100)}`;
  rtime.style.display='';rtime.innerHTML=`Time: <b${r._time>2000?' class="slow"':''}'>${r._time}ms</b>`;
  ...
  let pretty='';
  if(ct.includes('json')||/^\s*[\[{]/.test(r._body)){try{pretty=jsonHL(JSON.parse(r._body));}catch{pretty=esc(r._body);}}
  else{pretty=esc(r._body);}
  ...
  document.getElementById('r-headers-tbl').innerHTML=Object.entries(r._headers||{}).map(([k,v])=>`<tr><td>${esc(k)}</td><td>${esc(v)}</td></tr>`).join('')||`<tr><td colspan="2" style="color:var(--text3);padding:10px">No headers</td></tr>`;
  renderCookiesPanel();
}
...
function renderTests(){
  ...
  if(!_testResults.length){c.innerHTML='<div class="empty-state"><div class="ei">🧪</div><p>No tests ran. Write tests in the Tests tab.</p></div>';badge.style.display='none';return;}
...
function flushConsole(){
  document.getElementById('console-out').innerHTML=_consoleLogs.map(l=>`<div class="con-row ${l.type}"><span class="ct">${l.type}</span><span class="cm">${esc(l.msg)}</span></div>`).join('');
}
...
function switchRespBody(id){document.querySelectorAll('.rbview').forEach(b=>b.classList.toggle('active',b.dataset.view===id));document.querySelectorAll('.rbpanel').forEach(p=>p.classList.toggle('active',p.id==='rbp-'+id));}
...
// ─────────────────────────────────────────────────────────────
// HISTORY — ALL FIXES HERE
// 1. addHistory always sets pinned:false (never auto-pinned)
// 2. renderHistory uses data-* attributes, zero inline onclick
// 3. initHistoryEvents() delegates all events — pin, del, load
// 4. unpinAllHistory() correctly sets pinned:false & saves & re-renders
// 5. fixHistory() on load normalises old data
// ─────────────────────────────────────────────────────────────
...
function addHistory(entry){
  ...
  // Always pinned:false — user must pin manually
  ...
  const row=h=>{
    ...
    return `<div class="hist-row${p?' pinned':''}" data-hid="${h.id}">
      ...
      <div class="hist-acts">
        ...
        <button class="hist-del-btn" data-action="del" data-hid="${h.id}" title="Delete this entry">🗑</button>
      </div>
    </div>`;
  };
...
// Called once during init — delegates ALL history events
function initHistoryEvents(){
  ...
  list.addEventListener('click',function(e){
    ...
    if(pinBtn){
      ...
      if(!h)return;
      h.pinned=!h.pinned;          // true boolean toggle
      // Stable sort: pinned float to top, preserve insertion order within each group
    ...
    // Load request (click on row itself)
...
// Unpin All — sets every entry pinned:false, saves, re-renders
function unpinAllHistory(){
  const pinCount=S.history.filter(h=>h.pinned===true).length;
  if(!pinCount){notify('Nothing is pinned','info');return;}
  S.history.forEach(h=>{h.pinned=false;});
  save();
  renderHistory();
  notify(`Unpinned ${pinCount} item${pinCount!==1?'s':''}  ✓`,'success');
}
...
function runCollModal(e,id){
  e.stopPropagation();
  const coll=S.collections.find(c=>c.id===id);if(!coll?.requests?.length){notify('Collection is empty','error');return;}
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mf"><button class="btn secondary" onclick="closeModal()">Close</button><button class="btn primary" onclick="doRunColl('${id}')">▶ Run</button></div>
  </div></div>`);
}
...
async function doRunColl(id){
  ...
  for(let iter=0;iter<iters;iter++){for(const dataRow of dataRows){_iterInfo={iteration:iter,iterationCount:iters,dataRow};for(const req of reqs){
    try{
      const url=resolveVars(req.url||'',dataRow);const direct=isPrivate(url);const fUrl=direct?url:(S.settings.corsEnabled?S.settings.proxyUrl+encodeURIComponent(url):url);
      const h={};(req.headers||[]).filter(x=>x.on!==false&&(x.k||x.key)).forEach(x=>{h[resolveVars(x.k||x.key,dataRow)]=resolveVars(x.v||x.value||'',dataRow);});
      const fo={method:req.method||'GET',headers:h};if(req.rawBody&&!['GET','HEAD'].includes(req.method))fo.body=resolveVars(req.rawBody,dataRow);
      if(req.preScript?.trim()){const pm=buildPM(null,coll.variables||{});runScript(req.preScript,pm);}
      const r=await fetch(fUrl,fo);const body=await r.text();const respH={};r.headers.forEach((v,k)=>{respH[k]=v;});
      const ro={status:r.status,statusText:r.statusText,_body:body,_headers:respH,_time:0,_size:new Blob([body]).size};
      _testResults=[];if(req.testScript?.trim()){const pm=buildPM(ro,coll.variables||{});runScript(req.testScript,pm);}
      const pt=_testResults.filter(t=>t.pass).length,isOk=r.ok&&(!_testResults.length||pt===_testResults.length);
      if(isOk)passed++;else failed++;
      resultEl.innerHTML+=`<div class="cr-result-item ${isOk?'pass':'fail'}"><span>${isOk?'✅':'❌'}</span><div><div style="font-weight:600;font-size:12px">[${iter+1}/${iters}] ${esc(req.name)} — ${r.status} ${r.statusText}</div>${_testResults.map(t=>`<div style="font-size:11px;color:${t.pass?'var(--ok)':'var(--err)'};margin-top:2px">${t.pass?'✓':'✗'} ${esc(t.name)}${t.error?' — '+esc(t.error):''}</div>`).join('')}</div></div>`;
...
function openNewColl(){
  openModal(`<div class="modal-bg"><div class="modal sm"><div class="mh"><span class="mh-title">New Collection</span><button class="m-close" onclick="closeModal()">✕</button></div><div class="mb"><div class="fg"><label>NAME</label><input id="nc-name" placeholder="My Collection" autofocus></div><div class="fg"><label>DESCRIPTION</label><textarea id="nc-desc" rows="2" style="width:100%;resize:none" placeholder="Optional"></textarea></div></div><div class="mf"><button class="btn secondary" onclick="closeModal()">Cancel</button><button class="btn primary" onclick="createColl()">Create</button></div></div></div>`);
...
function doSave(){
  const name=document.getElementById('sr-name').value.trim(),id=document.getElementById('sr-coll').value;
  const coll=S.collections.find(c=>c.id===id),tab=getActiveTab();if(!coll||!name)return;
  if(!coll.requests)coll.requests=[];
  coll.requests.push({id:uid(),name,method:tab?.method||'GET',url:tab?.url||'',headers:tab?.headers||[],params:tab?.params||[],rawBody:tab?.rawBody||'',bodyType:tab?.bodyType||'none',rawFmt:tab?.rawFmt||'json',authType:tab?.authType||'none',authData:tab?.authData||{},preScript:tab?.preScript||'',testScript:tab?.testScript||''});
  if(tab)tab.name=name;save();renderCollections();renderTabs();closeModal();notify('Saved!','success');
}
function loadCollReq(cid,rid){
  const coll=S.collections.find(c=>c.id===cid);let req=coll?.requests?.find(r=>r.id===rid);
  if(!req){for(const item of coll?.requests||[]){if(item._isFolder){req=item.requests?.find(r=>r.id===rid);if(req)break;}}}
  if(!req)return;newTab({...req});
}
...
function exportColl(e,id){
  e.stopPropagation();const coll=S.collections.find(c=>c.id===id);if(!coll)return;
  dl(JSON.stringify({info:{name:coll.name,description:coll.desc||'',schema:'https://schema.getpostman.com/json/collection/v2.1.0/collection.json'},variable:Object.entries(coll.variables||{}).map(([k,v])=>({key:k,value:v})),item:(coll.requests||[]).map(r=>r._isFolder?{name:r.name,item:(r.requests||[]).map(mapToPostman)}:mapToPostman(r))},null,2),coll.name.replace(/\s+/g,'_')+'.postman_collection.json');
...
function renderEnvs(){
  ...
  list.innerHTML=S.envs.map(e=>`<div class="env-row${e.id===S.activeEnv?' active-env':''}" onclick="setEnv('${e.id}')"><div class="env-dot${e.id===S.activeEnv?' on':''}"></div><span class="env-nm">${esc(e.name)}</span><button class="btn-s" onclick="editEnv(event,'${e.id}')">Edit</button><button class="btn-s" onclick="exportEnv(event,'${e.id}')">⬇</button><button class="btn-s" onclick="delEnv(event,'${e.id}')">🗑</button></div>`).join('');
  refreshEnvQuick();
}
...
async function invokeGRPC(){
  const url=document.getElementById('grpc-url')?.value?.trim(),method=document.getElementById('grpc-method')?.value?.trim(),body=document.getElementById('grpc-body')?.value?.trim(),meta=document.getElementById('grpc-meta')?.value?.trim(),respEl=document.getElementById('grpc-resp');
  if(!url||!method){notify('URL and method required','error');return;}
  respEl.innerHTML='<span style="color:var(--text3)">⏳ Invoking...</span>';
  const direct=isPrivate(url);const proxyUrl=(!direct&&S.settings.corsEnabled)?S.settings.proxyUrl+encodeURIComponent(url+'/'+method):url+'/'+method;
  const h={'Content-Type':'application/grpc-web+json','x-grpc-web':'1'};if(meta){try{Object.assign(h,JSON.parse(meta));}catch{}}
  try{const r=await fetch(proxyUrl,{method:'POST',headers:h,body:body||'{}'});const txt=await r.text();let p;try{p=JSON.parse(txt);}catch{p=txt;}respEl.innerHTML=`<span style="color:var(--ok)">Status: ${r.status} ${r.statusText}</span>\n\n${esc(typeof p==='string'?p:JSON.stringify(p,null,2))}`;notify('gRPC: '+r.status,r.ok?'success':'error');}
...
// IMPORT
// ─────────────────────────────────────────────────────────────
function openImport(){openModal(`<div class="modal-bg"><div class="modal md"><div class="mh"><span class="mh-title">📥 Import</span><button class="m-close" onclick="closeModal()">✕</button></div><div class="mb"><div class="fg"><label>PASTE JSON (Postman Collection v2.1, Environment, OpenAPI) OR cURL COMMAND</label><textarea id="imp-txt" rows="10" style="width:100%;background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:10px;color:var(--text1);font-family:var(--mono);font-size:12px;resize:vertical" placeholder="Paste Postman Collection JSON, Environment JSON, OpenAPI JSON, or curl command..."></textarea></div><div class="fg"><label>OR UPLOAD FILE (.json, .yaml)</label><input type="file" id="imp-file" accept=".json,.yaml,.yml" onchange="loadImpFile(this)"></div></div><div class="mf"><button class="btn secondary" onclick="closeModal()">Cancel</button><button class="btn primary" onclick="doImport()">Import</button></div></div></div>`);}
...
function doImport(){
  ...
  try{
    ...
    if(data.values&&(data.name||data.id)){const env={id:uid(),name:data.name||'Imported Env',variables:{}};(data.values||[]).forEach(v=>{env.variables[v.key]=v.value;});S.envs.push(env);save();renderEnvs();closeModal();notify(`✅ Imported env "${env.name}"`,'success');return;}
...
function openSettings(){
  ...
  openModal(`<div class="modal-bg"><div class="modal lg"><div class="mh"><span class="mh-title">⚙ Settings</span><button class="m-close" onclick="closeModal()">✕</button></div><div class="mb">
    <div class="s-sec"><div class="s-sec-title">CORS PROXY</div>
      <div class="s-row"><div><div class="s-label">Enable CORS Proxy</div><div class="s-desc">Route public API requests through Cloudflare Worker. Private IPs (10.x, 192.168.x, etc.) always go DIRECT — no proxy needed.</div></div><label class="toggle"><input type="checkbox" id="set-cors"${s.corsEnabled?' checked':''} onchange="toggleCORSFromSettings()"><span class="t-slider"></span></label></div>
    ...
    <div class="s-sec"><div class="s-sec-title">THEME</div>
      <div class="s-row"><div><div class="s-label">Dark Mode</div><div class="s-desc">Toggle between dark and light themes</div></div><label class="toggle"><input type="checkbox" id="set-dark"${s.theme!=='light'?' checked':''} onchange="toggleThemeFromSettings(this)"><span class="t-slider"></span></label></div>
    </div>
    ...
    <div class="s-sec"><div class="s-sec-title">ABOUT</div>
      ...
      Private IPs (10.x, 192.168.x etc.) are always called DIRECTLY — no proxy needed.<br>
...
async function testProxy(){
  const purl=document.getElementById('set-proxy').value.trim(),res=document.getElementById('proxy-test-res');
  res.textContent='⏳ Testing...';res.style.color='var(--text3)';
  try{const r=await fetch(purl+encodeURIComponent('https://httpbin.org/get'),{signal:AbortSignal.timeout(8000)});if(r.ok){res.textContent='✅ Worker is working!';res.style.color='var(--ok)';}else{res.textContent=`⚠ Worker: ${r.status}`;res.style.color='var(--warn)';}}
...
function init(){
  ...
  renderAll();
  initResize();
  initHistoryEvents();   // ← sets up delegated event listener for history
  refreshCORSBtn();
  refreshHistDot();
