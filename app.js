...
const S = {
  ...
  history:     normalizeHistory(load('pw_hist', [])),   // ← normalize on load
...
// Ensure every history entry has pinned:false if it was never set
// This fixes the "all pinned by default" bug from old data
function normalizeHistory(arr) {
  return (arr || []).map(h => ({
    ...h,
    pinned: h.pinned === true   // only true if explicitly true
  }));
}
...
// Requests to these addresses bypass the CORS proxy entirely
// and go direct — supports http://10.x.x.x:port, 192.168.x.x, etc.
// ============================================================
...
function isPrivateUrl(urlStr) {
  try {
    ...
    return PRIVATE_PATTERNS.some(p => p.test(u.hostname));
  } catch { return false; }
}

// Update the DIRECT badge in the URL bar
function updateDirectBadge(urlStr) {
  const badge = document.getElementById('direct-badge');
  if (!badge) return;
...
function validateSchema(data, schema, path='#') {
  ...
  function chk(d, s, p) {
    ...
    if (s.type) {
      ...
      if (!types.includes(actual)) errors.push(`${p}: expected [${types}], got ${actual}`);
    }
    ...
    if (typeof d==='string') {
      ...
      if (s.pattern && !new RegExp(s.pattern).test(d)) errors.push(`${p}: pattern failed`);
      if (s.format==='email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d)) errors.push(`${p}: invalid email`);
      if (s.format==='uri'   && !/^https?:\/\/.+/.test(d))              errors.push(`${p}: invalid URI`);
    }
    if (typeof d==='number') {
      if (s.minimum!==undefined && d<s.minimum)     errors.push(`${p}: < minimum ${s.minimum}`);
      if (s.maximum!==undefined && d>s.maximum)     errors.push(`${p}: > maximum ${s.maximum}`);
      if (s.multipleOf!==undefined && d%s.multipleOf!==0) errors.push(`${p}: not multipleOf ${s.multipleOf}`);
    }
    if (Array.isArray(d)) {
    ...
    if (d!==null && typeof d==='object' && !Array.isArray(d)) {
      ...
      if (s.additionalProperties===false && s.properties)
        Object.keys(d).forEach(k=>{ if(!s.properties[k]) errors.push(`${p}: additional prop '${k}'`); });
    }
...
function buildPM(response, collVars={}) {
  ...
  const chai = (val) => {
    const self = {
      ...
      deep: {
        equal: x => { if(JSON.stringify(val)!==JSON.stringify(x)) throw new Error('Deep equal failed'); return self; }
      },
      not: {
        ...
        have: {
          property: p => { if(typeof val==='object'&&val!==null&&p in val) throw new Error(`Expected NOT to have "${p}"`); return self; }
        },
        be: {
          above: x => { if(val>x) throw new Error(`Expected <= ${x}`); return self; },
          below: x => { if(val<x) throw new Error(`Expected >= ${x}`); return self; },
        },
      },
      be: {
        ...
        oneOf:     arr=> { if(!arr.includes(val)) throw new Error(`Expected one of [${arr}]`); return self; },
        closeTo:   (x,d=2)=>{ if(Math.abs(val-x)>d) throw new Error(`Expected ${val} ≈ ${x} ±${d}`); return self; },
      },
      have: {
        property: (p,v) => {
          if(typeof val!=='object'||val===null||!(p in val)) throw new Error(`Expected property "${p}"`);
          if(v!==undefined&&val[p]!==v) throw new Error(`Property "${p}" expected ${JSON.stringify(v)}, got ${JSON.stringify(val[p])}`);
          return self;
        },
  ...
  const pmResp = response ? {
    ...
    json: () => {
      ...
      catch { throw new Error('Response is not valid JSON. Got: ' + String(response._body).slice(0,120)); }
    },
    text:    () => response._body || '',
    ...
    to: {
      ...
      be: {
        ...
        notFound:    () => { if(response.status!==404) throw new Error(`Not 404, got ${response.status}`); },
        created:     () => { if(response.status!==201) throw new Error(`Not 201, got ${response.status}`); },
        accepted:    () => { if(response.status!==202) throw new Error(`Not 202, got ${response.status}`); },
      },
      not: {
        have: { status: code=>{ if(response.status===code) throw new Error(`Status should NOT be ${code}`); } },
        be:   { ok: ()=>{ if(response.status>=200&&response.status<300) throw new Error('Should not be 2xx'); } },
      },
    },
  } : {
  ...
  const pm = {
    ...
    request: {
      ...
      headers: {
        ...
        upsert:   (k,v) => { const t=getActiveTab();if(!t)return;const ex=t.headers.find(h=>h.k===k);if(ex)ex.v=v;else t.headers.push({id:uid(),on:true,k,v,desc:''});loadKV('headers',t.headers); },
    ...
    sendRequest: (opts, cb) => {
      ...
      const direct  = isPrivateUrl(url);
      const fetchUrl = (!direct && S.settings.corsEnabled)
        ? S.settings.proxyUrl + encodeURIComponent(url)
        : url;
      const headers = {};
      if (opts.header) {
        if (Array.isArray(opts.header)) opts.header.forEach(h=>{headers[h.key]=h.value;});
        else Object.assign(headers, opts.header);
      }
      if (opts.headers) Object.assign(headers, opts.headers);
      const fo = { method:(opts.method||'GET').toUpperCase(), headers };
      if (opts.body) {
        fo.body = typeof opts.body==='string' ? opts.body
          : opts.body?.mode==='raw' ? opts.body.raw
          : JSON.stringify(opts.body);
      }
      fetch(fetchUrl, fo).then(async r => {
        const body=await r.text(); const hdrs={};
...
function newTab(d) {
  const t = mkTab(d);
  S.tabs.push(t); S.activeId = t.id;
  renderTabs(); loadTabUI(t); showResponse(null);
}

function switchTab(id) {
  saveTabUI(); S.activeId = id;
  const t = S.tabs.find(t=>t.id===id);
  loadTabUI(t); renderTabs(); showResponse(t?.response);
}
...
function closeTab(id, e) {
  ...
  loadTabUI(t); showResponse(t?.response); renderTabs();
}
...
function toggleFormType(sel) {
  const tr=sel.closest('tr'), file=sel.value==='file';
...
function readFormData() {
  ...
  document.querySelectorAll('#kv-form tr').forEach(tr => {
    ...
    if(type==='file') {
      ...
      rows.push({on:true,k:key,v:'',type:'file',file:f});
    } else {
      rows.push({on:true,k:key,v:tr.querySelector('.fv-text input')?.value||'',type:'text'});
    }
  });
  return rows;
}
...
function updatePathVars(url='', saved=[]) {
  const tbody=document.getElementById('kv-pathvars'), emptyEl=document.getElementById('pv-empty');
  if(!tbody) return;
  const found=[];
...
function readPathVars() {
  ...
  document.querySelectorAll('#kv-pathvars tr').forEach(tr=>{
    ...
    const inp=tr.querySelectorAll('input');
    rows.push({k, v:inp[0]?.value||'', desc:inp[1]?.value||''});
  });
  return rows;
}
...
function onRawFmtChange() {
  const fmt=document.getElementById('raw-fmt')?.value, ta=document.getElementById('code-raw');
  if(!ta||ta.value.trim()) return;
  const hints={json:'{"key":"value"}',xml:'<root>\n  <element>value</element>\n</root>',html:'<h1>Hello</h1>',text:'Your text',javascript:'console.log("hello")'};
  ta.placeholder=hints[fmt]||'';
}
...
const AUTH_FIELDS = {
  ...
  bearer: `<div class="af"><label>TOKEN</label><input type="text" id="a-token" placeholder="Bearer token (supports {{variable}} syntax)"></div>
    <p class="auth-info">Adds <code>Authorization: Bearer &lt;token&gt;</code> automatically.</p>`,
  apikey: `<div class="af"><label>KEY NAME</label><input type="text" id="a-key" placeholder="e.g. X-API-Key"></div>
    <div class="af"><label>KEY VALUE</label><input type="text" id="a-key-val" placeholder="your-api-key (supports {{variable}})"></div>
    <div class="af"><label>ADD TO</label><select id="a-key-in"><option value="header">Header</option><option value="query">Query Params</option></select></div>`,
  basic: `<div class="af"><label>USERNAME</label><input type="text" id="a-user" placeholder="username"></div>
    <div class="af"><label>PASSWORD</label><input type="password" id="a-pass" placeholder="password"></div>
    <p class="auth-info">Encodes as Base64 → <code>Authorization: Basic &lt;base64(user:pass)&gt;</code></p>`,
  digest: `<div class="af"><label>USERNAME</label><input type="text" id="a-du" placeholder="username"></div>
    <div class="af"><label>PASSWORD</label><input type="password" id="a-dp" placeholder="password"></div>
    <div class="af"><label>REALM (auto-detected from 401)</label><input type="text" id="a-realm" placeholder="leave blank for auto-detect"></div>
    <div class="af"><label>NONCE (auto-detected from 401)</label><input type="text" id="a-nonce" placeholder="leave blank for auto-detect"></div>
    <div class="af"><label>QOP</label><input type="text" id="a-qop" placeholder="auth (default)"></div>
    <p class="auth-info">💡 Leave realm/nonce blank — PostmanWeb auto-detects from the 401 challenge.</p>`,
  oauth1: `<div class="af"><label>CONSUMER KEY</label><input type="text" id="a-ck" placeholder="Consumer Key"></div>
    <div class="af"><label>CONSUMER SECRET</label><input type="text" id="a-cs" placeholder="Consumer Secret"></div>
    <div class="af"><label>ACCESS TOKEN</label><input type="text" id="a-at" placeholder="Access Token (optional)"></div>
    <div class="af"><label>TOKEN SECRET</label><input type="text" id="a-ts" placeholder="Token Secret (optional)"></div>
    <div class="af"><label>SIGNATURE METHOD</label><select id="a-sm"><option value="HMAC-SHA1">HMAC-SHA1</option><option value="HMAC-SHA256">HMAC-SHA256</option></select></div>
    <p class="auth-info">OAuth 1.0a signature (timestamp, nonce, HMAC) computed automatically per request.</p>`,
  oauth2: `<div class="af"><label>ACCESS TOKEN</label><input type="text" id="a-o2" placeholder="Paste your OAuth 2.0 access token"></div>
    <div class="af"><label>HEADER PREFIX</label><input type="text" id="a-o2p" value="Bearer" placeholder="Bearer"></div>
    <p class="auth-info">Obtain your token from your OAuth 2.0 provider and paste it above.<br>Adds: <code>Authorization: Bearer &lt;token&gt;</code></p>`,
  hawk: `<div class="af"><label>HAWK AUTH ID</label><input type="text" id="a-hid" placeholder="Hawk Auth ID"></div>
    <div class="af"><label>HAWK AUTH KEY</label><input type="text" id="a-hkey" placeholder="Hawk Auth Key"></div>
    <div class="af"><label>ALGORITHM</label><select id="a-halg"><option value="sha256">sha256</option><option value="sha1">sha1</option></select></div>
    <p class="auth-info">Hawk Authorization header is HMAC-signed automatically per request.</p>`,
  aws: `<div class="af"><label>ACCESS KEY ID</label><input type="text" id="a-ak" placeholder="AKIAIOSFODNN7EXAMPLE"></div>
    <div class="af"><label>SECRET ACCESS KEY</label><input type="password" id="a-sk" placeholder="wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"></div>
    <div class="af"><label>AWS REGION</label><input type="text" id="a-region" placeholder="us-east-1"></div>
    <div class="af"><label>SERVICE</label><input type="text" id="a-svc" placeholder="execute-api"></div>
    <div class="af"><label>SESSION TOKEN (optional)</label><input type="text" id="a-sess" placeholder="For temporary/assumed-role credentials"></div>
    <p class="auth-info">AWS Signature Version 4 computed and signed automatically per request.</p>`,
  ntlm: `<div class="af"><label>USERNAME</label><input type="text" id="a-nu" placeholder="username or DOMAIN\\username"></div>
    <div class="af"><label>PASSWORD</label><input type="password" id="a-np" placeholder="password"></div>
    <div class="af"><label>DOMAIN</label><input type="text" id="a-nd" placeholder="DOMAIN (optional)"></div>
    <div class="af"><label>WORKSTATION</label><input type="text" id="a-nw" placeholder="optional"></div>
    <p class="auth-info">⚠ NTLM requires multi-step challenge/response. Browser CORS restrictions may limit full handshake.</p>`,
};
...
async function computeAuth(method, url) {
  ...
  if(type==='bearer') {
    ...
    if(id&&key) { try{headers['Authorization']=await signHawk(method,url,id,key,alg);}catch(e){console.error('Hawk',e);} }
...
async function signOAuth1(method,url,ck,cs,at,ts,sm='HMAC-SHA1') {
  ...
  const qp={};urlObj.searchParams.forEach((v,k)=>{qp[k]=v;});
...
async function signAWSv4(method,url,body,ak,sk,region,service,session) {
  ...
  const signH={'host':u.hostname+(u.port?':'+u.port:''),'x-amz-date':dt,'x-amz-content-sha256':bodyHash,...(session?{'x-amz-security-token':session}:{})};
  ...
  const qpArr=[];u.searchParams.forEach((v,k)=>qpArr.push([encodeURIComponent(k),encodeURIComponent(v)]));
  ...
  return {'Authorization':`AWS4-HMAC-SHA256 Credential=${ak}/${scope}, SignedHeaders=${signedHdrs}, Signature=${sig}`,'x-amz-date':dt,'x-amz-content-sha256':bodyHash,...(session?{'x-amz-security-token':session}:{})};
}
...
// ============================================================
// SEND REQUEST
// Key fix: private/internal IPs always go DIRECT, never through proxy
// ============================================================
function cancelReq() {
  _abortCtrl?.abort();
...
async function sendRequest() {
  ...
  if(preCode.trim()) {
    ...
    runScript(preCode,pmObj); flushConsole();
  }

  // Resolve vars + path vars
  ...
  if(mock) {
    ...
    const fakeResp={
      status:mock.statusCode||200, statusText:'OK (Mock)',
    ...
    showResponse(fakeResp); renderTests(); flushConsole();
    notify(`🎭 Mock ${fakeResp.status} returned`,'info');
    return;
  }

  // ── ROUTING LOGIC ──────────────────────────────────────────
  // Private/internal IPs ALWAYS go direct (bypass proxy)
  // Everything else: use proxy if corsEnabled, else direct
  const isDirect = isPrivateUrl(finalUrl);
  const fetchUrl = isDirect
    ? finalUrl
    : (S.settings.corsEnabled
        ? S.settings.proxyUrl + encodeURIComponent(finalUrl)
        : finalUrl);
  ...
  try {
    ...
    if(document.getElementById('auth-sel')?.value==='digest') {
      ...
      if(r0?.status===401) {
        ...
        if(realm&&nonce) {
          ...
          let uriPath; try{uriPath=new URL(finalUrl).pathname;}catch{uriPath='/';}
    ...
    const responseObj={status:resp.status,statusText:resp.statusText,_body:respTxt,_headers:respH,_time:elapsed,_size:size};
    ...
    showResponse(responseObj); flushConsole(); renderTests();
    notify(`${resp.status} ${resp.statusText}  —  ${elapsed}ms`,
      resp.status>=500?'error':resp.status>=400?'warn':'success');
    ...
    if(e.name==='AbortError') {
      ...
      const hint = isDirect
        ? `${e.message}\n\n💡 This is a private/internal network address. Make sure the server is reachable from your browser's network.`
        : S.settings.corsEnabled
          ? e.message
    ...
    cancelBtn.style.display='none'; _abortCtrl=null;
  }
}
...
function renderTests() {
  ...
  if(!_testResults.length) {
    c.innerHTML='<div class="empty-state"><div class="ei">🧪</div><p>No tests ran. Write tests in the Tests tab.</p></div>';
...
// ============================================================
// HISTORY  — event-delegated, no inline onclick
// ============================================================
...
function renderHistory() {
  ...
  refreshHistDot();
  if(!S.history.length){
    list.innerHTML='<div class="empty-state"><div class="ei">📭</div><p>No history yet.</p></div>';
    return;
  }
  // Sort: pinned first, then by insertion order (already maintained)
  const pinned =S.history.filter(h=>h.pinned===true);
  const recent =S.history.filter(h=>h.pinned!==true);

  const row = h => {
    const pinned = h.pinned===true;
    return `<div class="hist-row${pinned?' pinned':''}" data-hid="${h.id}">
      ...
      <div class="hist-acts">
        <button class="btn-pin${pinned?' is-pinned':''}" data-action="pin" data-hid="${h.id}" title="${pinned?'Unpin':'Pin'}">${pinned?'📌':'📍'}</button>
        <button class="btn-del-hist" data-action="del" data-hid="${h.id}" title="Delete">🗑</button>
      </div>
    </div>`;
  };
...
// Single delegated listener on hist-list — handles load, pin, delete
function initHistDelegation() {
  const list=document.getElementById('hist-list');
  list.addEventListener('click', e => {
    // Find which button or row was clicked
    ...
    if(pinBtn) {
      e.stopPropagation();
      const id=pinBtn.dataset.hid;
      const h=S.history.find(x=>x.id===id);
      if(!h) return;
      h.pinned = h.pinned !== true;          // toggle
    ...
    // Clicking the row itself = load request
    if(row) {
      const id=row.dataset.hid;
      const h=S.history.find(x=>x.id===id);
      if(h) newTab({method:h.method,url:h.url,name:h.url.replace(/^https?:\/\//,'').slice(0,40)||'Request'});
    }
  });
}
...
function unpinAll() {
  ...
  save(); renderHistory(); notify('All items unpinned','info');
}
...
function renderCollItem(c) {
  const items=(c.requests||[]).map(item=>
    item._isFolder
      ? `<div class="coll-folder">
           <div class="folder-header" onclick="toggleFolder('${c.id}','${item.id}')">
             <span class="folder-arrow" id="fa-${item.id}">▶</span>📁 ${esc(item.name)}
           </div>
  ...
  return `<div class="coll-item" id="coll-${c.id}">
    ...
    <div class="coll-reqs" id="cr-${c.id}">
      ${items||'<div style="padding:8px;color:var(--text3);font-size:11px">Empty collection</div>'}
    </div>
  </div>`;
}
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      ...
      <div style="margin-bottom:10px"><div class="field-label">SELECT REQUESTS</div>
        ${reqs.map(r=>`<div class="cr-req-item"><input type="checkbox" class="cr-req-chk kv-chk" data-rid="${r.id}" checked>
          <span class="mbadge ${r.method}" style="color:${MC[r.method]||'var(--text2)'}">${r.method}</span>
...
async function doRunColl(id) {
  ...
  if(dataFile){
    ...
    } catch(e){notify('Data file error: '+e.message,'error');return;}
  }
    ...
    for(const dataRow of dataRows){
      ...
      for(const req of reqs){
        try {
          ...
          const direct=isPrivateUrl(url);
          const fUrl=direct?url:(S.settings.corsEnabled?S.settings.proxyUrl+encodeURIComponent(url):url);
          const h={};
          (req.headers||[]).filter(x=>x.on!==false&&(x.k||x.key)).forEach(x=>{h[resolveVars(x.k||x.key,dataRow)]=resolveVars(x.v||x.value||'',dataRow);});
          const fo={method:req.method||'GET',headers:h};
          if(req.rawBody&&!['GET','HEAD'].includes(req.method))fo.body=resolveVars(req.rawBody,dataRow);
          if(req.preScript?.trim()){const pm=buildPM(null,coll.variables||{});runScript(req.preScript,pm);}
          const r=await fetch(fUrl,fo);
          ...
          const passedT=_testResults.filter(t=>t.pass).length;
          const isOk=r.ok&&(!_testResults.length||passedT===_testResults.length);
        ...
        const bar=document.getElementById('cr-bar');if(bar)bar.style.width=Math.round(done/total*100)+'%';
...
function openNewColl(){
  openModal(`<div class="modal-bg"><div class="modal sm">
    ...
    <div class="mb">
      ...
      <div class="fg"><label>DESCRIPTION</label><textarea id="nc-desc" rows="2" style="width:100%;resize:none" placeholder="Optional"></textarea></div>
    </div>
...
function delColl(e,id){
  e.stopPropagation();if(!confirm('Delete this collection?'))return;
...
function saveToCollection(){
  saveTabUI();const tab=getActiveTab();
  ...
  openModal(`<div class="modal-bg"><div class="modal sm">
    ...
    <div class="mb">
      ...
      <div class="fg"><label>COLLECTION</label><select id="sr-coll" style="width:100%">${S.collections.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select></div>
    </div>
...
function exportColl(e,id){
  e.stopPropagation();
  const coll=S.collections.find(c=>c.id===id);if(!coll)return;
  const data={info:{name:coll.name,description:coll.desc||'',schema:'https://schema.getpostman.com/json/collection/v2.1.0/collection.json'},
    variable:Object.entries(coll.variables||{}).map(([k,v])=>({key:k,value:v})),
    item:(coll.requests||[]).map(r=>r._isFolder?{name:r.name,item:(r.requests||[]).map(mapReqToPostman)}:mapReqToPostman(r))};
...
function quickEnvSwitch(id){
  S.activeEnv=id||null;save();renderEnvs();
  notify(id?`Env: ${S.envs.find(e=>e.id===id)?.name}`:'No environment active','info');
}
function setEnv(id){
  S.activeEnv=S.activeEnv===id?null:id;save();renderEnvs();
  const e=S.envs.find(e=>e.id===S.activeEnv);
  notify(S.activeEnv?`Env: ${e?.name}`:'No environment active','info');
}
function openNewEnv(){
  openModal(`<div class="modal-bg"><div class="modal sm">
...
function createEnv(){
  ...
  S.envs.push(env);save();renderEnvs();closeModal();editEnv(null,env.id);
}
function editEnv(e,id){
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      <div id="ev-list">
        ${vars.map(([k,v])=>`<div class="ev-row"><input placeholder="Variable" value="${esc(k)}"><input placeholder="Value" value="${esc(v)}"><button class="ev-del" onclick="this.parentElement.remove()">✕</button></div>`).join('')}
      </div>
...
function saveEnv(id){
  const env=S.envs.find(x=>x.id===id);if(!env)return;
  env.variables={};
  document.querySelectorAll('#ev-list .ev-row').forEach(row=>{const[k,v]=row.querySelectorAll('input');if(k.value.trim())env.variables[k.value.trim()]=v.value;});
...
function delEnv(e,id){
  ...
  S.envs=S.envs.filter(x=>x.id!==id);if(S.activeEnv===id)S.activeEnv=null;
  save();renderEnvs();
}
function exportEnv(e,id){
  e.stopPropagation();
  const env=S.envs.find(x=>x.id===id);if(!env)return;
  dl(JSON.stringify({id:env.id,name:env.name,values:Object.entries(env.variables||{}).map(([k,v])=>({key:k,value:v,enabled:true}))},null,2),env.name.replace(/\s+/g,'_')+'.postman_environment.json');
}
...
function openGlobals(){
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      <div id="gv-list">
        ${Object.entries(S.globals).map(([k,v])=>`<div class="ev-row"><input placeholder="Variable" value="${esc(k)}"><input placeholder="Value" value="${esc(v)}"><button class="ev-del" onclick="this.parentElement.remove()">✕</button></div>`).join('')}
      </div>
...
function saveGlobals(){
  S.globals={};
  document.querySelectorAll('#gv-list .ev-row').forEach(row=>{const[k,v]=row.querySelectorAll('input');if(k.value.trim())S.globals[k.value.trim()]=v.value;});
...
function openCodegen(){
  ...
  openModal(`<div class="modal-bg"><div class="modal xl">
    ...
    <div class="mb">
      <div class="lang-tabs">
        ${['cURL','JavaScript (Fetch)','JavaScript (Axios)','Python (requests)','Java (OkHttp)','C# (HttpClient)','Go (net/http)','PHP (Guzzle)','Ruby (Net::HTTP)','Swift','Kotlin','Rust (reqwest)','Node.js','PowerShell']
...
function genCode(lang){
  ...
  const authType=document.getElementById('auth-sel')?.value;
  const authH={};
  ...
  const codes={
    ...
    'Go (net/http)':()=>{ const bline=hasBody?`strings.NewReader(${JSON.stringify(rawBody)})`:'nil';const hStr=Object.entries(allH).map(([k,v])=>`  req.Header.Add("${k}", "${v}")`).join('\n');return `package main\n\nimport (\n  "fmt"\n  "net/http"\n  "io/ioutil"\n  "strings"\n)\n\nfunc main() {\n  client := &http.Client{}\n  payload := ${bline}\n  req, _ := http.NewRequest("${method}", "${url}", payload)\n${hStr}\n  res, _ := client.Do(req)\n  defer res.Body.Close()\n  body, _ := ioutil.ReadAll(res.Body)\n  fmt.Println(string(body))\n}`; },
    ...
    'Rust (reqwest)':()=>{ const hStr=Object.entries(allH).map(([k,v])=>`.header("${k}", "${v}")`).join('\n    ');return `use reqwest;\n\n#[tokio::main]\nasync fn main() -> Result<(), Box<dyn std::error::Error>> {\n    let client = reqwest::Client::new();\n    let res = client.${method.toLowerCase()}("${url}")\n    ${hStr}\n    ${hasBody?`.body(${JSON.stringify(rawBody)})\n    `:''}.send().await?;\n    println!("{}", res.text().await?);\n    Ok(())\n}`; },
...
function wsSend(){
  const msg=document.getElementById('ws-msg')?.value?.trim();if(!msg)return;
...
function openGRPC(){
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      ...
      <div id="grpc-resp" style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);min-height:80px;max-height:200px;overflow-y:auto;padding:10px;font-family:var(--mono);font-size:12px;color:var(--text3);margin-top:8px">gRPC response will appear here...</div>
    </div>
    <div class="mf"><button class="btn secondary" onclick="closeModal()">Close</button><button class="btn primary" onclick="invokeGRPC()">Invoke</button></div>
  </div></div>`);
}
async function invokeGRPC(){
  ...
  respEl.innerHTML='<span style="color:var(--text3)">⏳ Invoking...</span>';
  const direct=isPrivateUrl(url);
  const proxyUrl=(!direct&&S.settings.corsEnabled)?S.settings.proxyUrl+encodeURIComponent(url+'/'+method):url+'/'+method;
  const headers={'Content-Type':'application/grpc-web+json','x-grpc-web':'1'};
  ...
  }catch(e){respEl.innerHTML=`<span style="color:var(--err)">${esc(e.message)}</span>`;notify('gRPC error: '+e.message,'error');}
}
...
function openMockServer(){
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      <p style="font-size:12px;color:var(--text3);margin-bottom:12px">Enable "Use Mock Response" in Request Settings to intercept matching requests.</p>
      <div id="mock-rules">${S.mocks.length?S.mocks.map((m,i)=>mockRuleHtml(m,i)).join(''):'<p style="color:var(--text3);font-size:12px">No mock rules yet.</p>'}</div>
      <button class="add-row-btn" style="margin-top:10px" onclick="addMockRule()">+ Add Mock Rule</button>
    </div>
    <div class="mf"><button class="btn secondary" onclick="closeModal()">Close</button><button class="btn primary" onclick="saveMockRules()">Save Rules</button></div>
  </div></div>`);
}
function mockRuleHtml(m,i){
  return `<div class="mock-rule" id="mock-rule-${i}">
    <div class="mock-rule-hdr" style="margin-bottom:8px">
      <select style="width:90px" id="mr-method-${i}">${['*','GET','POST','PUT','PATCH','DELETE','HEAD','OPTIONS'].map(x=>`<option${x===m.method?' selected':''}>${x}</option>`).join('')}</select>
      <input type="text" id="mr-path-${i}" value="${esc(m.path||'')}" placeholder="/api/users" style="flex:1">
      <select id="mr-status-${i}" style="width:80px">${[200,201,204,400,401,403,404,422,500,502,503].map(s=>`<option${s===m.statusCode?' selected':''}>${s}</option>`).join('')}</select>
    ...
    <div class="fg"><label>RESPONSE BODY</label><textarea id="mr-body-${i}" class="code-area" rows="4">${esc(m.body||'{}')}</textarea></div>
    <div class="fg"><label>CONTENT TYPE</label><input type="text" id="mr-ct-${i}" value="${esc(m.contentType||'application/json')}"></div>
  </div>`;
}
  ...
  openModal(`<div class="modal-bg"><div class="modal md">
    ...
    <div class="mb">
      <div class="fg"><label>PASTE JSON (Postman Collection v2.1, Environment, OpenAPI) OR cURL COMMAND</label>
        <textarea id="imp-txt" rows="10" style="width:100%;background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:10px;color:var(--text1);font-family:var(--mono);font-size:12px;resize:vertical" placeholder="Paste Postman Collection JSON, Environment JSON, OpenAPI JSON, or curl command..."></textarea>
      </div>
      <div class="fg"><label>OR UPLOAD FILE (.json, .yaml)</label><input type="file" id="imp-file" accept=".json,.yaml,.yml" onchange="loadImpFile(this)"></div>
    </div>
  ...
  try{
    ...
    if(data.info&&data.item){
      ...
      const flat=(items,target)=>items?.forEach(item=>{
        if(item.item){const folder={id:uid(),name:item.name,_isFolder:true,requests:[]};flat(item.item,folder.requests);target.push(folder);}
        else if(item.request){target.push({id:uid(),name:item.name||'Request',method:item.request.method||'GET',
          url:typeof item.request.url==='string'?item.request.url:(item.request.url?.raw||''),
          headers:(item.request.header||[]).map(h=>({id:uid(),on:!h.disabled,k:h.key,v:h.value,desc:h.description||''})),
          rawBody:item.request.body?.raw||'',bodyType:item.request.body?.mode==='raw'?'raw':item.request.body?.mode||'none',rawFmt:'json',
    ...
    if(data.values&&(data.name||data.id)){
      ...
      S.envs.push(env);save();renderEnvs();closeModal();notify(`✅ Imported env "${env.name}"`,'success');return;
    }
    if(Array.isArray(data)&&data[0]?.requests){
      S.collections.push(...data);save();renderCollections();closeModal();notify(`Imported ${data.length} collections`,'success');return;
    }
  ...
  return d.map(domain=>
    ...
    Object.entries(S.cookies[domain]).map(([k,v])=>
      `<div class="ck-row"><span class="ck-name">${esc(k)}</span><span class="ck-val">${esc(v)}</span>
       <button onclick="delCK('${esc(domain)}','${esc(k)}')" style="color:var(--err);background:none;border:none;cursor:pointer;margin-left:auto">✕</button></div>`
    ).join('')+'</div>'
  ).join('');
}
function addCK(){
  ...
  if(!S.cookies[d])S.cookies[d]={};S.cookies[d][n]=v;save();
  document.getElementById('ck-list').innerHTML=renderCKList();notify('Cookie added!','success');
}
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      <div class="s-sec">
        ...
        <div class="s-row">
          <div><div class="s-label">Enable CORS Proxy</div><div class="s-desc">Route requests through Cloudflare Worker. Private IPs (10.x, 192.168.x, etc.) always bypass the proxy and go direct.</div></div>
      ...
      <div class="s-sec">
        ...
        <p style="font-size:12px;color:var(--text3);line-height:1.8">PostmanWeb v4 — Full API Testing Platform in your browser.<br>
        All data stored locally in localStorage. No server, no login required.<br>
        Private/internal IPs always called directly (no proxy needed).<br>
  ...
  inp.onchange=e=>{
    ...
    r.onload=ev=>{
      try{
        ...
        if(d.collections)S.collections=d.collections;if(d.envs)S.envs=d.envs;
        if(d.globals)S.globals=d.globals;if(d.history)S.history=normalizeHistory(d.history);
        if(d.settings)S.settings=d.settings;if(d.mocks)S.mocks=d.mocks;
...
function initResize(){
  ...
  handle.addEventListener('mousedown',e=>{dragging=true;sy=e.clientY;sh=document.getElementById('req-area').offsetHeight;document.body.style.userSelect='none';document.body.style.cursor='ns-resize';});
  document.addEventListener('mousemove',e=>{if(!dragging)return;const nh=Math.max(80,Math.min(wrap.offsetHeight-80,sh+(e.clientY-sy)));document.getElementById('req-area').style.height=nh+'px';});
...
function init(){
  ...
  renderAll();
  initResize();
  initHistDelegation();     // ← event delegation for history (fixes pin/unpin bug)
  refreshCORSBtn();
  refreshHistDot();
  ...
  // URL input: auto-name tab + update path vars + DIRECT badge
  document.getElementById('url-in').addEventListener('input',e=>{
    ...
    if(tab&&e.target.value){
      ...
      updateDirectBadge(e.target.value);
    }
  });
