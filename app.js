/* ============================================================
   POSTMANWEB v4 — app.js  (Complete Application Logic)
...
let _abortCtrl   = null;
let _wsConn      = null;
let _localVars   = {};          // pm.variables scope (request-scoped)
...
// CRYPTO UTILITIES
// ============================================================
async function _hmacSign(algo, key, data) {
  const enc = new TextEncoder();
  const keyData = typeof key === 'string' ? enc.encode(key) : key;
  const ck = await crypto.subtle.importKey('raw', keyData, { name:'HMAC', hash:algo }, false, ['sign']);
  return new Uint8Array(await crypto.subtle.sign('HMAC', ck, enc.encode(data)));
}
async function hmacB64(algo, key, data) {
  const b = await _hmacSign(algo, key, data);
  return btoa(String.fromCharCode(...b));
}
async function hmacHex(algo, key, data) {
  const b = await _hmacSign(algo, key, data);
...
function md5(str) {
  ...
  const arr=enc(str);const r=core(arr,str.length*8);
...
const DYN_VARS = {
  ...
  '$randomFullName':   () => DYN_VARS['$randomFirstName']() + ' ' + DYN_VARS['$randomLastName'](),
  '$randomEmail':      () => `user${Math.floor(Math.random()*90000+10000)}@example.com`,
  '$randomUrl':        () => `https://example${Math.floor(Math.random()*100)}.com`,
  '$randomDomainName': () => `domain${Math.floor(Math.random()*100)}.com`,
  '$randomIP':         () => [1,2,3,4].map(()=>Math.floor(Math.random()*255)).join('.'),
  '$randomIPV6':       () => [1,2,3,4,5,6,7,8].map(()=>Math.floor(Math.random()*65535).toString(16)).join(':'),
  '$randomPhoneNumber':() => `+1-${Math.floor(Math.random()*900+100)}-${Math.floor(Math.random()*900+100)}-${Math.floor(Math.random()*9000+1000)}`,
  '$randomPassword':   () => btoa(Math.random().toString(36)+Math.random().toString(36)).slice(0,16),
  '$randomWords':      () => ['lorem ipsum','dolor sit','amet consectetur','adipiscing elit'][Math.floor(Math.random()*4)],
  '$randomNoun':       () => ['cat','dog','car','tree','house','book','phone'][Math.floor(Math.random()*7)],
  '$randomAdjective':  () => ['big','small','fast','slow','happy','sad','bright'][Math.floor(Math.random()*7)],
  '$randomVerb':       () => ['run','jump','fly','swim','read','write','code'][Math.floor(Math.random()*7)],
  '$randomCountry':    () => ['India','USA','UK','Germany','France','Japan','Brazil','Canada','Australia','Spain'][Math.floor(Math.random()*10)],
  '$randomCity':       () => ['Mumbai','New York','London','Berlin','Paris','Tokyo','Sydney','Toronto'][Math.floor(Math.random()*8)],
  '$randomStreetName': () => `${Math.floor(Math.random()*999)+1} ${['Main','Oak','Pine','Maple','Cedar'][Math.floor(Math.random()*5)]} Street`,
  ...
  '$randomProductName':() => ['Widget Pro','Gizmo X','Thing Plus','Doohickey','SuperTool'][Math.floor(Math.random()*5)],
  ...
  '$randomDateRecent': () => new Date(Date.now()-Math.random()*7*86400000).toISOString().slice(0,10),
  '$randomImageUrl':   () => `https://picsum.photos/id/${Math.floor(Math.random()*1000)}/640/480`,
  '$randomAvatarImage':() => `https://i.pravatar.cc/150?u=${Math.random()}`,
  '$randomLoremWord':  () => ['lorem','ipsum','dolor','sit','amet','consectetur','adipiscing','elit'][Math.floor(Math.random()*8)],
  '$randomLoremSentence':() => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.',
  '$randomLoremParagraph':() => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.',
  '$randomHexColor':   () => '#'+Math.floor(Math.random()*16777215).toString(16).padStart(6,'0'),
  '$randomMACAddress': () => [1,2,3,4,5,6].map(()=>Math.floor(Math.random()*255).toString(16).padStart(2,'0')).join(':'),
  '$randomCreditCardMask':() => `xxxx-xxxx-xxxx-${Math.floor(Math.random()*9000+1000)}`,
...
function resolveVars(str, extra={}) {
  ...
  const cv  = tab?.collVars || {};
  // Dynamic vars  {{$xxx}}
  str = str.replace(/\{\{\s*(\$[a-zA-Z]+)\s*\}\}/g, (m, k) => {
    const fn = DYN_VARS[k.slice(1)] || DYN_VARS[k];
    return fn ? fn() : m;
  });
  // Named vars: local > collVars > env > globals > dataRow > extra
  str = str.replace(/\{\{([^}]+?)\}\}/g, (m, k) => {
    k = k.trim();
    return _localVars[k] ?? cv[k] ?? ev[k] ?? S.globals[k] ?? _iterInfo.dataRow[k] ?? extra[k] ?? m;
  });
  return str;
}
...
function validateSchema(data, schema, path='#') {
  ...
  function chk(d, s, p) {
    ...
    if (s.type) {
      ...
      if (!types.includes(actual)) errors.push(`${p}: expected type [${types}], got ${actual}`);
    }
    if ('const' in s && JSON.stringify(d)!==JSON.stringify(s.const))
      errors.push(`${p}: expected const ${JSON.stringify(s.const)}`);
    if (s.enum && !s.enum.some(v=>JSON.stringify(v)===JSON.stringify(d)))
      errors.push(`${p}: not in enum`);
    if (typeof d==='string') {
      if (s.minLength!==undefined && d.length<s.minLength) errors.push(`${p}: minLength ${s.minLength}`);
      if (s.maxLength!==undefined && d.length>s.maxLength) errors.push(`${p}: maxLength ${s.maxLength}`);
      if (s.pattern && !new RegExp(s.pattern).test(d)) errors.push(`${p}: pattern /${s.pattern}/ not matched`);
      if (s.format==='email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d)) errors.push(`${p}: invalid email`);
      if (s.format==='uri'   && !/^https?:\/\/.+/.test(d))              errors.push(`${p}: invalid URI`);
    }
    if (typeof d==='number') {
      if (s.minimum!==undefined && d<s.minimum)               errors.push(`${p}: min ${s.minimum}`);
      if (s.maximum!==undefined && d>s.maximum)               errors.push(`${p}: max ${s.maximum}`);
      if (s.exclusiveMinimum!==undefined && d<=s.exclusiveMinimum) errors.push(`${p}: exclusiveMin`);
      if (s.exclusiveMaximum!==undefined && d>=s.exclusiveMaximum) errors.push(`${p}: exclusiveMax`);
      if (s.multipleOf!==undefined && d%s.multipleOf!==0)     errors.push(`${p}: not multiple of ${s.multipleOf}`);
    }
    if (Array.isArray(d)) {
      if (s.minItems!==undefined && d.length<s.minItems) errors.push(`${p}: minItems ${s.minItems}`);
      if (s.maxItems!==undefined && d.length>s.maxItems) errors.push(`${p}: maxItems ${s.maxItems}`);
      if (s.uniqueItems && new Set(d.map(JSON.stringify)).size!==d.length) errors.push(`${p}: not unique`);
      if (s.items) d.forEach((x,i)=>chk(x,s.items,`${p}[${i}]`));
    }
    if (d!==null && typeof d==='object' && !Array.isArray(d)) {
      (s.required||[]).forEach(k=>{ if(!(k in d)) errors.push(`${p}: missing required '${k}'`); });
      if (s.minProperties!==undefined && Object.keys(d).length<s.minProperties)
        errors.push(`${p}: minProperties ${s.minProperties}`);
      if (s.maxProperties!==undefined && Object.keys(d).length>s.maxProperties)
        errors.push(`${p}: maxProperties ${s.maxProperties}`);
      if (s.properties) Object.entries(s.properties).forEach(([k,ps])=>{ if(k in d) chk(d[k],ps,`${p}.${k}`); });
      if (s.additionalProperties===false && s.properties)
        Object.keys(d).forEach(k=>{ if(!s.properties[k]) errors.push(`${p}: extra prop '${k}'`); });
    }
    if (s.allOf) s.allOf.forEach((sub,i)=>chk(d,sub,`${p}(allOf[${i}])`));
    if (s.anyOf && !s.anyOf.some(sub=>{ const prev=errors.length; chk(d,sub,p); if(errors.length>prev){errors.splice(prev);return false;}return true; }))
      errors.push(`${p}: does not match anyOf`);
    if (s.if) { const prev=errors.length; chk(d,s.if,p); if(errors.length===prev&&s.then) chk(d,s.then,p); else if(errors.length>prev){errors.splice(prev);if(s.else)chk(d,s.else,p);} }
  }
  chk(data, schema, path);
...
function buildPM(response, collVars={}) {
  ...
  // Chai-style assertion chain
  const chai = (val) => {
    const self = {
      _val: val,
      equal:       x => { if(val!==x) throw new Error(`Expected ${JSON.stringify(x)}, got ${JSON.stringify(val)}`); return self; },
      eql:         x => { if(JSON.stringify(val)!==JSON.stringify(x)) throw new Error(`Deep equal failed\nExpected: ${JSON.stringify(x)}\nGot: ${JSON.stringify(val)}`); return self; },
      ...
      keys:        arr => { arr.forEach(k=>{ if(typeof val!=='object'||!(k in val)) throw new Error(`Missing key: ${k}`); }); return self; },
      ...
      not: {
        ...
        null:    () => { if(val===null)           throw new Error('Expected not null'); return self; },
        be:      { above:(x)=>{ if(val>x) throw new Error(`Expected <= ${x}`); return self; }, below:(x)=>{ if(val<x) throw new Error(`Expected >= ${x}`); return self; } },
        have: { property: p => { if(typeof val==='object'&&val!==null&&p in val) throw new Error(`Expected NOT to have property "${p}"`); return self; } },
      },
      be: {
        below:  x  => { if(!(val<x))  throw new Error(`Expected ${val} < ${x}`);  return self; },
        above:  x  => { if(!(val>x))  throw new Error(`Expected ${val} > ${x}`);  return self; },
        at: {
          least: x => { if(!(val>=x)) throw new Error(`Expected ${val} >= ${x}`); return self; },
          most:  x => { if(!(val<=x)) throw new Error(`Expected ${val} <= ${x}`); return self; },
        },
        ...
        boolean:   () => { if(typeof val!=='boolean') throw new Error('Expected boolean'); return self; },
        ...
        oneOf:     arr=> { if(!arr.includes(val)) throw new Error(`Expected one of [${arr}], got ${val}`); return self; },
        ok:        () => { if(!val) throw new Error('Expected truthy'); return self; },
        closeTo:   (x,d=2) => { if(Math.abs(val-x)>d) throw new Error(`Expected ${val} close to ${x} ±${d}`); return self; },
      },
      have: {
        property: (p,v) => {
          if(typeof val!=='object'||val===null||!(p in val)) throw new Error(`Expected property "${p}"`);
          if(v!==undefined && val[p]!==v) throw new Error(`Expected property "${p}" = ${JSON.stringify(v)}, got ${JSON.stringify(val[p])}`);
          return self;
        },
        ...
        lengthAbove: n=>{ if(!val||val.length<=n)   throw new Error(`Expected length > ${n}`); return self; },
        status:    code => {
        ...
        header:    (key,value) => {
          if(!response) throw new Error('No response');
          const hv = response._headers?.[key.toLowerCase()];
          if(!hv) throw new Error(`Missing header: ${key}`);
          if(value!==undefined && hv!==String(value)) throw new Error(`Header "${key}" expected "${value}", got "${hv}"`);
          return self;
        },
        jsonBody:  path => {
          try {
            const body = JSON.parse(response._body);
            const v = path.split('.').reduce((o,k)=>o?.[k], body);
        ...
        members: arr => { if(!Array.isArray(val)) throw new Error('Expected array'); arr.forEach(m=>{ if(!val.includes(m)) throw new Error(`Missing member: ${m}`); }); return self; },
      },
    };
    self.to  = self;
    self.and = self;
  ...
  const pmResp = response ? {
    ...
    json: () => {
      try { return JSON.parse(response._body); }
      catch(e) { throw new Error('Response is not valid JSON. Preview: ' + String(response._body).slice(0,120)); }
    },
    text: () => response._body || '',
    ...
    to: {
      ...
      be: {
        ok:      () => { if(response.status<200||response.status>=300) throw new Error(`Status ${response.status} is not OK`); },
        json:    () => { try{JSON.parse(response._body);}catch{throw new Error('Not JSON');} },
        success: () => { if(response.status<200||response.status>=300) throw new Error(`Not success: ${response.status}`); },
        error:   () => { if(response.status<400) throw new Error(`Not an error: ${response.status}`); },
        ...
        accepted: () => { if(response.status!==202) throw new Error(`Not 202`); },
        notFound: () => { if(response.status!==404) throw new Error(`Not 404`); },
        forbidden:() => { if(response.status!==403) throw new Error(`Not 403`); },
        created:  () => { if(response.status!==201) throw new Error(`Not 201`); },
      },
      not: {
        have: { status: code => { if(response.status===code) throw new Error(`Expected NOT ${code}`); } },
        be:   { ok: () => { if(response.status>=200&&response.status<300) throw new Error(`Unexpected 2xx`); } },
      },
    },
  } : {
    code:0, status:'', responseTime:0, size:0,
    json:()=>({}), text:()=>'', cookies:{},
    headers:{get:()=>null,has:()=>false,toObject:()=>({}),all:()=>({})},
    to:{ have:{status:()=>{},header:()=>{},jsonBody:()=>{}}, be:{ok:()=>{},json:()=>{},success:()=>{},error:()=>{}},not:{have:{status:()=>{}},be:{ok:()=>{}}} },
  };
  ...
  const pm = {
    ...
    test: (name, fn) => {
      try {
        ...
        if (r instanceof Promise) {
          r.catch(e => _testResults.push({ name, pass:false, error:e.message }))
           .then(() => _testResults.push({ name, pass:true }));
        } else {
          _testResults.push({ name, pass:true });
        }
      } catch(e) {
        _testResults.push({ name, pass:false, error:e.message });
      }
    },
    expect: chai,
    ...
    request: {
      url: {
        ...
        getQuery: () => { try{ return new URL(resolveVars(document.getElementById('url-in')?.value||'')).search; }catch{ return ''; } },
        addQueryParams: params => {
          const t = getActiveTab();
          if(t) { Object.entries(params).forEach(([k,v])=>t.params.push({id:uid(),on:true,k,v,desc:''})); loadKV('params',t.params); }
        },
      },
      ...
      headers: {
        ...
        upsert: (k,v) => {
          ...
          const ex=t.headers.find(h=>h.k===k);
          if(ex) ex.v=v; else t.headers.push({id:uid(),on:true,k,v,desc:''});
    ...
    environment: {
      ...
      has:      k  => env?.variables?.[k] !== undefined,
    ...
    // ── variables (local, request-scoped) ─────────────────
    variables: {
    ...
    // ── info ──────────────────────────────────────────────
    info: {
      iteration:      _iterInfo.iteration,
      iterationCount: _iterInfo.iterationCount,
    ...
    sendRequest: (opts, cb) => {
      ...
      const fetchUrl = S.settings.corsEnabled ? S.settings.proxyUrl + encodeURIComponent(url) : url;
      const headers = {};
      if (opts.header) {
        if (Array.isArray(opts.header))
          opts.header.forEach(h=>{ headers[h.key]=h.value; });
        else
          Object.assign(headers, opts.header);
      }
      if (opts.headers) Object.assign(headers, opts.headers);
      const fetchOpts = { method:(opts.method||'GET').toUpperCase(), headers };
      if (opts.body) {
        fetchOpts.body = typeof opts.body==='string' ? opts.body
          : opts.body?.mode==='raw' ? opts.body.raw
          : JSON.stringify(opts.body);
      }
      fetch(fetchUrl, fetchOpts).then(async r => {
        const body = await r.text();
        const hdrs = {};
        r.headers.forEach((v,k)=>{ hdrs[k]=v; });
        const res = {
          code:r.status, status:r.statusText, _body:body, _headers:hdrs,
          json:()=>JSON.parse(body), text:()=>body,
          headers:{ get:k=>hdrs[k.toLowerCase()], has:k=>!!(hdrs[k.toLowerCase()]), toObject:()=>({...hdrs}) },
          to:{ have:{ status:code=>{ if(r.status!==code)throw new Error(`${r.status}!=${code}`); } } },
        };
        if (cb) cb(null, res);
...
function runScript(code, pmObj) {
  ...
  const con = {
    ...
    assert:(cond,msg)=>{ if(!cond) _consoleLogs.push({type:'error',msg:'Assertion failed: '+(msg||'')}); },
  };
  try {
    new Function('pm','console','expect','require', code)(
      pmObj.pm, con, pmObj.expect,
      mod => { _consoleLogs.push({type:'warn',msg:`require('${mod}') is not supported in browser`}); return {}; }
    );
  } catch(e) {
    _consoleLogs.push({type:'error', msg:'Script error: '+e.message});
  }
}
...
const METHOD_COLORS = {
  ...
  PATCH:'var(--patch)', DELETE:'var(--delete)', HEAD:'var(--head)', OPTIONS:'var(--options)'
};

function renderTabs() {
  document.getElementById('tabs').innerHTML = S.tabs.map(t =>
    `<div class="tab-item${t.id===S.activeId?' active':''}" onclick="switchTab('${t.id}')">
       <span class="tab-method" style="color:${METHOD_COLORS[t.method]||'var(--text2)'}">${t.method}</span>
       <span class="tab-name">${esc(t.name)}</span>
       <button class="tab-close" onclick="closeTab('${t.id}',event)" title="Close">✕</button>
     </div>`
  ).join('');
}
...
// ============================================================
// KV TABLES (params / headers / urlencoded)
// ============================================================
...
function readKV(type) {
  ...
  document.querySelectorAll('#kv-'+type+' tr').forEach(tr => {
    ...
    if(inp.length >= 3)
      rows.push({ id:tr.dataset.id||uid(), on:inp[0].type==='checkbox'?inp[0].checked:true, k:inp[1]?.value||'', v:inp[2]?.value||'', desc:inp[3]?.value||'' });
  });
  return rows;
}
...
function addFormRow(k='', v='', type='text') {
  ...
  tr.innerHTML = `
    ...
    <td>
      <select class="fv-type-sel" onchange="toggleFormType(this)">
...
function toggleFormType(sel) {
  ...
  const file = sel.value === 'file';
  tr.querySelector('.fv-text').style.display = file ? 'none' : '';
  tr.querySelector('.fv-file').style.display = file ? '' : 'none';
}
...
function readFormData() {
  ...
  document.querySelectorAll('#kv-form tr').forEach(tr => {
    ...
    const key  = tr.querySelectorAll('input[type=text]')[0]?.value || '';
    const type = tr.querySelector('.fv-type-sel')?.value || 'text';
    if(!chk?.checked || !key) return;
    if(type==='file') {
      const file = tr.querySelector('.fv-file input[type=file]')?.files?.[0];
      rows.push({on:true, k:key, v:'', type:'file', file});
    } else {
      const val = tr.querySelector('.fv-text input')?.value || '';
      rows.push({on:true, k:key, v:val, type:'text'});
    }
  });
  return rows;
}
...
function loadFormData(rows=[]) {
  ...
  if(!tbody) return;
  tbody.innerHTML = '';
  rows.forEach(r => addFormRow(r.k||'', r.v||'', r.type||'text'));
...
function updatePathVars(url='', saved=[]) {
  const tbody   = document.getElementById('kv-pathvars');
  const emptyEl = document.getElementById('pv-empty');
  if(!tbody) return;
  const found = [];
  [/:([a-zA-Z_][a-zA-Z0-9_]*)(?=\/|$|\?|#)/g, /\{([a-zA-Z_][a-zA-Z0-9_]*)\}/g].forEach(re => {
    let m; while((m=re.exec(url))!==null) if(!found.includes(m[1])) found.push(m[1]);
  });
  tbody.innerHTML = '';
  const savedMap = Object.fromEntries(saved.map(r=>[r.k,r]));
  found.forEach(p => {
    const sv = savedMap[p]||{};
    const tr = document.createElement('tr');
    tr.dataset.key = p;
    tr.innerHTML = `
      <td style="padding:3px 6px;font-family:var(--mono);font-size:12px;color:var(--accent);white-space:nowrap">:${esc(p)}</td>
      <td><input type="text" placeholder="value or {{variable}}" value="${esc(sv.v||'')}"></td>
      <td><input type="text" placeholder="Description"           value="${esc(sv.desc||'')}"></td>
      <td></td>`;
    tbody.appendChild(tr);
  });
  if(emptyEl) emptyEl.style.display = found.length ? 'none' : '';
}

function readPathVars() {
  const rows = [];
  document.querySelectorAll('#kv-pathvars tr').forEach(tr => {
    const k = tr.dataset.key || ''; if(!k) return;
    const inps = tr.querySelectorAll('input');
    rows.push({ k, v:inps[0]?.value||'', desc:inps[1]?.value||'' });
  });
  return rows;
}
...
function resolvePathInUrl(url) {
  readPathVars().forEach(row => {
    if(!row.k) return;
    const val = encodeURIComponent(resolveVars(row.v));
...
function beautifyRaw() {
  const ta  = document.getElementById('code-raw');
  const fmt = document.getElementById('raw-fmt')?.value;
  if(!ta?.value.trim()) return;
  if(fmt==='json') {
    try { ta.value = JSON.stringify(JSON.parse(ta.value), null, 2); notify('Beautified ✨','success'); }
    catch(e) { notify('Invalid JSON: '+e.message,'error'); }
  } else {
    notify('Beautify only supported for JSON currently','info');
  }
}
...
function onRawFmtChange() {
  ...
  const ta  = document.getElementById('code-raw');
  if(!ta) return;
  const hints = { json:'{"key": "value"}', xml:'<root>\n  <element>value</element>\n</root>', html:'<h1>Hello World</h1>', text:'Your text here', javascript:'console.log("hello");' };
  if(!ta.value.trim()) ta.placeholder = hints[fmt] || '';
}

function showBinFile(input) {
  const f = input.files?.[0];
  if(f) document.getElementById('bin-label').textContent = `📎 ${f.name}  (${formatBytes(f.size)})`;
}

function handleBinDrop(e) {
  e.preventDefault();
  document.getElementById('bin-drop').classList.remove('dov');
  const f = e.dataTransfer?.files?.[0];
  if(!f) return;
  const dt = new DataTransfer(); dt.items.add(f);
  document.getElementById('bin-file').files = dt.files;
...
// ============================================================
const AUTH_FIELDS = {
  none: `<p class="auth-info">This request has no authorization. Select an auth type above.</p>`,

  bearer: `<div class="af"><label>TOKEN</label>
    <input type="text" id="a-token" placeholder="Bearer token (supports {{variable}} syntax)"></div>
    <p class="auth-info">Adds <code>Authorization: Bearer &lt;token&gt;</code> header automatically.</p>`,

  apikey: `<div class="af"><label>KEY NAME</label>
    <input type="text" id="a-key" placeholder="e.g. X-API-Key"></div>
    <div class="af"><label>KEY VALUE</label>
    <input type="text" id="a-key-val" placeholder="your-api-key (supports {{variable}} syntax)"></div>
    <div class="af"><label>ADD TO</label>
    <select id="a-key-in"><option value="header">Header</option><option value="query">Query Params</option></select></div>`,

  basic: `<div class="af"><label>USERNAME</label>
    <input type="text" id="a-user" placeholder="username"></div>
    <div class="af"><label>PASSWORD</label>
    <input type="password" id="a-pass" placeholder="password"></div>
    <p class="auth-info">Credentials encoded as Base64 → <code>Authorization: Basic &lt;base64(user:pass)&gt;</code></p>`,

  digest: `<div class="af"><label>USERNAME</label>
    <input type="text" id="a-du" placeholder="username"></div>
    <div class="af"><label>PASSWORD</label>
    <input type="password" id="a-dp" placeholder="password"></div>
    <div class="af"><label>REALM (optional – auto from 401)</label>
    <input type="text" id="a-realm" placeholder="leave blank for auto-detect"></div>
    <div class="af"><label>NONCE (optional – auto from 401)</label>
    <input type="text" id="a-nonce" placeholder="leave blank for auto-detect"></div>
    <div class="af"><label>QOP</label>
    <input type="text" id="a-qop" placeholder="auth (default)"></div>
    <p class="auth-info">💡 PostmanWeb will auto-detect realm & nonce from the 401 challenge response.</p>`,

  oauth1: `<div class="af"><label>CONSUMER KEY</label>
    <input type="text" id="a-ck" placeholder="Consumer Key"></div>
    <div class="af"><label>CONSUMER SECRET</label>
    <input type="text" id="a-cs" placeholder="Consumer Secret"></div>
    <div class="af"><label>ACCESS TOKEN</label>
    <input type="text" id="a-at" placeholder="Access Token (optional)"></div>
    <div class="af"><label>TOKEN SECRET</label>
    <input type="text" id="a-ts" placeholder="Token Secret (optional)"></div>
    <div class="af"><label>SIGNATURE METHOD</label>
    <select id="a-sm"><option value="HMAC-SHA1">HMAC-SHA1</option><option value="HMAC-SHA256">HMAC-SHA256</option></select></div>
    <p class="auth-info">OAuth 1.0a signature computed automatically per request (timestamp, nonce, HMAC).</p>`,

  oauth2: `<div class="af"><label>ACCESS TOKEN</label>
    <input type="text" id="a-o2" placeholder="Paste your OAuth 2.0 access token here"></div>
    <div class="af"><label>HEADER PREFIX</label>
    <input type="text" id="a-o2p" value="Bearer" placeholder="Bearer"></div>
    <p class="auth-info">OAuth 2.0 token acquisition is server-side. Obtain your token from your provider and paste above.<br>
    Adds: <code>Authorization: Bearer &lt;token&gt;</code></p>`,

  hawk: `<div class="af"><label>HAWK AUTH ID</label>
    <input type="text" id="a-hid" placeholder="Hawk Auth ID"></div>
    <div class="af"><label>HAWK AUTH KEY</label>
    <input type="text" id="a-hkey" placeholder="Hawk Auth Key"></div>
    <div class="af"><label>ALGORITHM</label>
    <select id="a-halg"><option value="sha256">sha256</option><option value="sha1">sha1</option></select></div>
    <p class="auth-info">Hawk Authorization header signed automatically with timestamp, nonce and HMAC.</p>`,

  aws: `<div class="af"><label>ACCESS KEY ID</label>
    <input type="text" id="a-ak" placeholder="AKIAIOSFODNN7EXAMPLE"></div>
    <div class="af"><label>SECRET ACCESS KEY</label>
    <input type="password" id="a-sk" placeholder="wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"></div>
    <div class="af"><label>AWS REGION</label>
    <input type="text" id="a-region" placeholder="us-east-1"></div>
    <div class="af"><label>SERVICE</label>
    <input type="text" id="a-svc" placeholder="execute-api"></div>
    <div class="af"><label>SESSION TOKEN (optional)</label>
    <input type="text" id="a-sess" placeholder="For temporary/assumed-role credentials"></div>
    <p class="auth-info">AWS Signature Version 4 computed and signed automatically per request.</p>`,

  ntlm: `<div class="af"><label>USERNAME</label>
    <input type="text" id="a-nu" placeholder="username or DOMAIN\\username"></div>
    <div class="af"><label>PASSWORD</label>
    <input type="password" id="a-np" placeholder="password"></div>
    <div class="af"><label>DOMAIN</label>
    <input type="text" id="a-nd" placeholder="DOMAIN (optional)"></div>
    <div class="af"><label>WORKSTATION</label>
    <input type="text" id="a-nw" placeholder="optional"></div>
    <p class="auth-info">⚠ NTLM requires multi-step challenge/response. Browser CORS restrictions may limit full NTLM handshake.</p>`,
};

function renderAuthFields(data={}) {
  const type = document.getElementById('auth-sel')?.value || 'none';
  document.getElementById('auth-fields').innerHTML = AUTH_FIELDS[type] || '';
  const tab = getActiveTab();
  const ad  = (data && Object.keys(data).length > 0) ? data : (tab?.authData || {});
  document.querySelectorAll('#auth-fields input, #auth-fields select').forEach(el => {
...
// Async – returns { headers:{}, queryParams:{} }
async function computeAuth(method, url) {
  const type = document.getElementById('auth-sel')?.value || 'none';
  const headers = {}, queryParams = {};

  if(type==='bearer') {
    const t = resolveVars(document.getElementById('a-token')?.value?.trim()||'');
    if(t) headers['Authorization'] = 'Bearer '+t;

  } else if(type==='basic') {
    const u = resolveVars(document.getElementById('a-user')?.value||'');
    const p = resolveVars(document.getElementById('a-pass')?.value||'');
    headers['Authorization'] = 'Basic ' + btoa(unescape(encodeURIComponent(u+':'+p)));

  } else if(type==='apikey') {
    const loc = document.getElementById('a-key-in')?.value;
    const k   = resolveVars(document.getElementById('a-key')?.value?.trim()||'');
    const v   = resolveVars(document.getElementById('a-key-val')?.value||'');
    if(k && v) { if(loc==='query') queryParams[k]=v; else headers[k]=v; }

  } else if(type==='oauth2') {
    const t = resolveVars(document.getElementById('a-o2')?.value?.trim()||'');
    const p = document.getElementById('a-o2p')?.value || 'Bearer';
    if(t) headers['Authorization'] = p+' '+t;

  } else if(type==='oauth1') {
    const ck = document.getElementById('a-ck')?.value?.trim()||'';
    const cs = document.getElementById('a-cs')?.value?.trim()||'';
    const at = document.getElementById('a-at')?.value?.trim()||'';
    const ts = document.getElementById('a-ts')?.value?.trim()||'';
    const sm = document.getElementById('a-sm')?.value||'HMAC-SHA1';
    if(ck && cs) {
      try { headers['Authorization'] = await signOAuth1(method, url, ck, cs, at, ts, sm); }
      catch(e) { console.error('OAuth1 sign error', e); }
    }

  } else if(type==='hawk') {
    const id  = document.getElementById('a-hid')?.value?.trim()||'';
    const key = document.getElementById('a-hkey')?.value?.trim()||'';
    const alg = document.getElementById('a-halg')?.value||'sha256';
    if(id && key) {
      try { headers['Authorization'] = await signHawk(method, url, id, key, alg); }
      catch(e) { console.error('Hawk sign error', e); }
    }

  } else if(type==='aws') {
    const ak  = document.getElementById('a-ak')?.value?.trim()||'';
    const sk  = document.getElementById('a-sk')?.value?.trim()||'';
    const reg = document.getElementById('a-region')?.value?.trim()||'us-east-1';
    const svc = document.getElementById('a-svc')?.value?.trim()||'execute-api';
    const ses = document.getElementById('a-sess')?.value?.trim()||'';
    if(ak && sk) {
      try { Object.assign(headers, await signAWSv4(method, url, null, ak, sk, reg, svc, ses)); }
      catch(e) { console.error('AWS sign error', e); }
    }
  }
...
async function signOAuth1(method, url, ck, cs, at, ts, sm='HMAC-SHA1') {
  let urlObj; try { urlObj=new URL(url); } catch { urlObj=new URL('https://example.com'); }
  const baseUrl = urlObj.protocol+'//'+urlObj.host+urlObj.pathname;
  const qp = {};
  urlObj.searchParams.forEach((v,k)=>{ qp[k]=v; });
  const nonce = Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2);
  const ts2   = String(Math.floor(Date.now()/1000));
  const op = { oauth_consumer_key:ck, oauth_nonce:nonce, oauth_signature_method:sm,
               oauth_timestamp:ts2, oauth_version:'1.0', ...(at?{oauth_token:at}:{}) };
  const allP  = {...qp, ...op};
  const pStr  = Object.keys(allP).sort().map(k=>`${pct(k)}=${pct(allP[k])}`).join('&');
  const base  = [method.toUpperCase(), pct(baseUrl), pct(pStr)].join('&');
  const sigKey = `${pct(cs)}&${pct(ts||'')}`;
  const algo   = sm.includes('256') ? 'SHA-256' : 'SHA-1';
  const sig    = await hmacB64(algo, sigKey, base);
...
async function signHawk(method, url, id, key, algo='sha256') {
  ...
  const resource = p.pathname+(p.search||'');
...
async function signAWSv4(method, url, body, ak, sk, region, service, session) {
  ...
  const dt   = now.toISOString().replace(/[:\-]|\.\d{3}/g,'').slice(0,15)+'Z';
  ...
  const sortedNames = Object.keys(signH).sort();
  const canonHdrs   = sortedNames.map(k=>`${k}:${signH[k]}`).join('\n')+'\n';
  const signedHdrs  = sortedNames.join(';');
  const qpArr = [];
  u.searchParams.forEach((v,k)=>qpArr.push([encodeURIComponent(k),encodeURIComponent(v)]));
  qpArr.sort(([a],[b])=>a<b?-1:a>b?1:0);
  const canonQS = qpArr.map(([k,v])=>`${k}=${v}`).join('&');
  const canonReq = [method.toUpperCase(), u.pathname||'/', canonQS, canonHdrs, signedHdrs, bodyHash].join('\n');
  const scope    = `${date}/${region}/${service}/aws4_request`;
  const sts      = ['AWS4-HMAC-SHA256', dt, scope, await sha256Hex(canonReq)].join('\n');
  const kDate    = await _hmacSign('SHA-256','AWS4'+sk, date);
  const kRegion  = await _hmacSign('SHA-256', kDate,   region);
  const kService = await _hmacSign('SHA-256', kRegion, service);
  const kSign    = await _hmacSign('SHA-256', kService,'aws4_request');
  const sigBytes = await _hmacSign('SHA-256', kSign,   sts);
  const sig      = [...sigBytes].map(b=>b.toString(16).padStart(2,'0')).join('');
  return {
    'Authorization': `AWS4-HMAC-SHA256 Credential=${ak}/${scope}, SignedHeaders=${signedHdrs}, Signature=${sig}`,
    'x-amz-date':    dt,
    'x-amz-content-sha256': bodyHash,
    ...(session?{'x-amz-security-token':session}:{}),
  };
...
function colorMethod() {
  ...
  if(!sel) return;
  sel.style.color = METHOD_COLORS[sel.value] || 'var(--text1)';
  const t = getActiveTab();
...
function toggleCORS() {
  ...
  save(); refreshCORSBtn();
  notify(S.settings.corsEnabled ? '⚡ CORS Proxy ENABLED' : '🔴 CORS Proxy disabled',
         S.settings.corsEnabled ? 'success' : 'info');
}
function refreshCORSBtn() {
  const btn = document.getElementById('cors-btn');
  if(!btn) return;
  btn.textContent = S.settings.corsEnabled ? '⚡ CORS: ON' : '⚡ CORS: OFF';
  btn.className   = S.settings.corsEnabled ? 'on' : '';
  btn.id          = 'cors-btn';
}
...
function checkMock(method, url) {
  ...
  for(const m of S.mocks) {
    ...
    const rx = new RegExp('^'+m.path.replace(/[.*+?^${}()|[\]\\]/g,'\\$&').replace(/\\\*/g,'.*')+'$');
    if(rx.test(url) || url.includes(m.path)) return m;
  }
  return null;
}
...
async function sendRequest() {
  ...
  if(preCode.trim()) {
    const pmObj = buildPM(null, tab?.collVars||{});
    runScript(preCode, pmObj);
    flushConsole();
  }

  // ── Resolve variables + path vars ──────────────────────
  let url = resolveVars(rawUrl);
  url = resolvePathInUrl(url);
  ...
  let finalUrl = url;
  const qpAll  = {
    ...Object.fromEntries(paramRows.map(r=>[resolveVars(r.k), resolveVars(r.v)])),
    ...authQP
  };
  ...
  if(!disableBody && !['GET','HEAD'].includes(method)) {
    if(_bodyType==='raw') {
      ...
      document.querySelectorAll('#kv-form tr').forEach(tr => {
        const chk = tr.querySelector('.kv-chk');
        const key = tr.querySelectorAll('input[type=text]')[0]?.value;
        const typ = tr.querySelector('.fv-type-sel')?.value||'text';
        if(!chk?.checked || !key) return;
        if(typ==='file') {
          const f = tr.querySelector('.fv-file input[type=file]')?.files?.[0];
          if(f) fd.append(key, f);
        } else {
          fd.append(key, resolveVars(tr.querySelector('.fv-text input')?.value||''));
        }
      });
      body = fd;
    } else if(_bodyType==='graphql') {
      let vars={};
      try{ vars=JSON.parse(resolveVars(document.getElementById('gql-v').value||'{}')); }catch{}
  ...
  if(mock) {
    ...
    const fakeResp = {
      ...
      statusText:  'OK (Mock)',
      _body:       resolveVars(mock.body||'{}'),
      _headers:    { 'content-type':mock.contentType||'application/json',
                     ...Object.fromEntries((mock.headers||[]).filter(h=>h.k).map(h=>[h.k.toLowerCase(),h.v])) },
      _time:       mock.delay||0,
      _size:       new Blob([mock.body||'']).size,
      _mock:       true,
    };
    if(tab) tab.response = fakeResp;
    const pmObj = buildPM(fakeResp, tab?.collVars||{});
    const testCode = document.getElementById('test-script').value;
    if(testCode.trim()) runScript(testCode, pmObj);
    showResponse(fakeResp);
    renderTests(); flushConsole();
    notify(`🎭 Mock ${fakeResp.status} returned`,'info');
    return;
  }
  ...
  try {
    ...
    if(document.getElementById('auth-sel')?.value==='digest') {
      ...
      if(r0?.status===401) {
        ...
        const u2    = document.getElementById('a-du')?.value||'';
        const p2    = document.getElementById('a-dp')?.value||'';
        if(realm&&nonce) {
          const nc=nc||'00000001'; const cnonce=Math.random().toString(36).slice(2,10);
          let uriPath; try{ uriPath=new URL(finalUrl.includes('://')?finalUrl:'https://x.com'+finalUrl).pathname; }catch{ uriPath='/'; }
          const ha1 = md5(`${u2}:${realm}:${p2}`);
          const ha2 = md5(`${method}:${uriPath}`);
          const res = md5(`${ha1}:${nonce}:${nc}:${cnonce}:${qop}:${ha2}`);
          headers['Authorization'] = `Digest username="${u2}", realm="${realm}", nonce="${nonce}", uri="${uriPath}", nc=${nc}, cnonce="${cnonce}", qop=${qop}, response="${res}"`;
    ...
    try {
      ...
      const sc = resp.headers.get('set-cookie')||respH['set-cookie']||'';
      if(sc) {
        if(!S.cookies[domain]) S.cookies[domain]={};
        sc.split(/,(?=[^;]+=[^;]+)/).forEach(c => {
          const [kv] = c.trim().split(';');
          const [ck,...cv] = kv.split('=');
          if(ck?.trim()) S.cookies[domain][ck.trim()] = cv.join('=').trim();
        });
        save();
      }
    } catch{}

    const responseObj = { status:resp.status, statusText:resp.statusText,
    ...
    const testCode = document.getElementById('test-script').value;
    if(testCode.trim()) {
      const pmObj = buildPM(responseObj, tab?.collVars||{});
      runScript(testCode, pmObj);
    }

    addHistory({ method, url:rawUrl, status:resp.status, time:elapsed });
    showResponse(responseObj);
    flushConsole(); renderTests();

    const sc = resp.status;
    notify(`${sc} ${resp.statusText}  —  ${elapsed}ms`,
           sc>=500?'error': sc>=400?'warn': 'success');

  } catch(e) {
    clearTimeout(tId);
    if(e.name==='AbortError') {
      notify('Request cancelled / timed out','info');
    } else {
      const hint = useProxy ? e.message
        : `${e.message}\n\n💡 Enable ⚡ CORS Proxy to bypass browser CORS restrictions.`;
      showErrorResp(hint, Date.now()-t0);
      notify('Request failed — '+(useProxy?e.message:'Enable CORS Proxy ⚡'), 'error');
    }
  } finally {
...
function showResponse(r) {
  const pill=document.getElementById('r-pill'), rtime=document.getElementById('r-time'),
        rsize=document.getElementById('r-size'), hint=document.getElementById('r-hint'),
        acts=document.getElementById('r-acts');
  if(!r) {
  ...
  pill.className = `spill ${r._mock?'smock':'s'+sc}`;
  // Time
  rtime.style.display='';
...
function renderTests() {
  ...
  if(!_testResults.length) {
    c.innerHTML='<div class="empty-state"><div class="ei">🧪</div><p>No tests ran.<br>Write tests in the Tests tab.</p></div>';
  ...
  const icon = pass===_testResults.length?'✅':pass===0?'❌':'⚠️';
  c.innerHTML =
...
function flushConsole() {
  document.getElementById('console-out').innerHTML = _consoleLogs.map(l =>
    `<div class="con-row ${l.type}">
       <span class="ct">${l.type}</span>
...
function renderCookiesPanel() {
  const p=document.getElementById('cookies-out');
  ...
  p.innerHTML = domains.map(d =>
    `<div class="ck-domain">
       ...
       ${Object.entries(S.cookies[d]).map(([k,v])=>
         `<div class="ck-row">
            <span class="ck-name">${esc(k)}</span>
            <span class="ck-val" title="${esc(v)}">${esc(v)}</span>
          </div>`
       ).join('')}
     </div>`
  ).join('');
...
function renderHistory() {
  ...
  const pinned=S.history.filter(h=>h.pinned), recent=S.history.filter(h=>!h.pinned);
  const row = h =>
    `<div class="hist-row${h.pinned?' pinned':''}" onclick="loadHist('${h.id}')">
       <span class="mbadge ${h.method}" style="color:${METHOD_COLORS[h.method]||'var(--text2)'}">${h.method}</span>
       ...
       <div class="hist-acts" onclick="event.stopPropagation()">
         <button class="icon-btn" title="${h.pinned?'Unpin':'Pin'}" onclick="pinHist('${h.id}')">${h.pinned?'📌':'📍'}</button>
...
function pinHist(id)   { const h=S.history.find(x=>x.id===id); if(!h) return; h.pinned=!h.pinned; S.history.sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0)); save(); renderHistory(); }
function delHist(id)   { S.history=S.history.filter(x=>x.id!==id); save(); renderHistory(); }
...
function unpinAll()    { S.history.forEach(h=>h.pinned=false); save(); renderHistory(); notify('All unpinned','info'); }
function loadHist(id)  { const h=S.history.find(x=>x.id===id); if(!h) return; newTab({method:h.method,url:h.url,name:h.url.slice(0,40)||'Request'}); }
...
function renderCollections() {
  ...
  if(!filtered.length) { list.innerHTML='<div class="empty-state"><div class="ei">📂</div><p>No collections yet.<br>Create or import one.</p></div>'; return; }
  list.innerHTML = filtered.map(c => renderCollItem(c)).join('');
}

function renderCollItem(c) {
  const reqs    = (c.requests||[]).filter(r=>!r._isFolder);
  const folders = (c.requests||[]).filter(r=>r._isFolder);
  const reqRows = (c.requests||[]).map(item =>
    item._isFolder
      ? `<div class="coll-folder">
           <div class="folder-header" onclick="toggleFolder('${c.id}','${item.id}')">
             <span class="folder-arrow" id="fa-${item.id}">▶</span>
             📁 ${esc(item.name)}
           </div>
           <div class="folder-reqs" id="fr-${item.id}">
             ${(item.requests||[]).map(r=>reqRowHtml(c.id,r,true)).join('')}
           </div>
         </div>`
      : reqRowHtml(c.id, item, false)
  ).join('');
  return `
    <div class="coll-item" id="coll-${c.id}">
      <div class="coll-header" onclick="toggleColl('${c.id}')">
        ...
        <div class="coll-btns">
          <button class="icon-btn" title="Run collection" onclick="runCollModal(event,'${c.id}')">▶</button>
      ...
      <div class="coll-reqs" id="cr-${c.id}">
        ${reqRows || '<div style="padding:8px;color:var(--text3);font-size:11px">Empty collection</div>'}
      </div>
    </div>`;
}
...
function reqRowHtml(collId, r, inFolder=false) {
  return `<div class="req-row${inFolder?' folder-req-row':''}" onclick="loadCollReq('${collId}','${r.id}')">
    <span class="mbadge ${r.method}" style="color:${METHOD_COLORS[r.method]||'var(--text2)'}">${r.method}</span>
...
function runCollModal(e, id) {
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    <div class="mh"><span class="mh-title">▶ Collection Runner — ${esc(coll.name)}</span><button class="m-close" onclick="closeModal()">✕</button></div>
    <div class="mb">
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:16px">
        ...
        <div class="fg"><label>STOP ON ERROR</label>
          <label class="toggle" style="margin-top:6px"><input type="checkbox" id="cr-stop"><span class="t-slider"></span></label>
        </div>
      </div>
      <div class="fg"><label>DATA FILE (CSV/JSON)</label>
        <input type="file" id="cr-data" accept=".json,.csv">
      </div>
      <div style="margin-bottom:10px"><div class="field-label">SELECT REQUESTS</div>
        ${reqs.map(r=>`<div class="cr-req-item">
          <input type="checkbox" class="cr-req-chk kv-chk" data-rid="${r.id}" checked>
          <span class="mbadge ${r.method}" style="color:${METHOD_COLORS[r.method]||'var(--text2)'}">${r.method}</span>
    ...
    <div class="mf">
      ...
      <button class="btn primary" onclick="doRunColl('${id}')">▶ Run</button>
    </div>
  </div></div>`);
}
...
async function doRunColl(id) {
  ...
  const iters = parseInt(document.getElementById('cr-iter')?.value)||1;
  const delay = parseInt(document.getElementById('cr-delay')?.value)||0;
  const stop  = document.getElementById('cr-stop')?.checked;
  ...
  // Parse data file if any
  let dataRows = [{}];
  const dataFile = document.getElementById('cr-data')?.files?.[0];
  if(dataFile) {
    try {
      const txt = await dataFile.text();
      if(dataFile.name.endsWith('.json')) {
        dataRows = JSON.parse(txt); if(!Array.isArray(dataRows)) dataRows=[dataRows];
      } else {
        // CSV parse
    ...
    } catch(e) { notify('Data file parse error: '+e.message,'error'); return; }
  }

  const total   = iters * dataRows.length * reqs.length;
  let done=0, passed=0, failed=0;
  const resultEl = document.getElementById('cr-results');
  resultEl.innerHTML = '';

  for(let iter=0; iter<iters; iter++) {
    for(const dataRow of dataRows) {
      _iterInfo = { iteration:iter, iterationCount:iters, dataRow };
      for(const req of reqs) {
        try {
          const url = resolveVars(req.url||'', dataRow);
          const useP = S.settings.corsEnabled;
          const fUrl = useP ? S.settings.proxyUrl+encodeURIComponent(url) : url;
          const h = {};
          (req.headers||[]).filter(x=>x.on!==false&&(x.k||x.key)).forEach(x=>{ h[resolveVars(x.k||x.key,dataRow)]=resolveVars(x.v||x.value||'',dataRow); });
          const fOpts = { method:req.method||'GET', headers:h };
          if(req.rawBody && !['GET','HEAD'].includes(req.method)) fOpts.body=resolveVars(req.rawBody,dataRow);

          // Pre-request script
          if(req.preScript?.trim()) { const pm=buildPM(null,coll.variables||{}); runScript(req.preScript,pm); }

          const r = await fetch(fUrl, fOpts);
          const body = await r.text();
          const respH = {}; r.headers.forEach((v,k)=>{ respH[k]=v; });
          const respObj = { status:r.status, statusText:r.statusText, _body:body, _headers:respH, _time:0, _size:new Blob([body]).size };

          // Test script
          _testResults=[];
          if(req.testScript?.trim()) { const pm=buildPM(respObj,coll.variables||{}); runScript(req.testScript,pm); }

          const passedTests = _testResults.filter(t=>t.pass).length;
          const isOk = r.ok && (!_testResults.length || passedTests===_testResults.length);
          if(isOk) passed++; else failed++;

          resultEl.innerHTML += `<div class="cr-result-item ${isOk?'pass':'fail'}">
            <span>${isOk?'✅':'❌'}</span>
            <div>
              <div style="font-weight:600;font-size:12px">[${iter+1}/${iters}] ${esc(req.name)} — ${r.status} ${r.statusText}</div>
              ${_testResults.map(t=>`<div style="font-size:11px;color:${t.pass?'var(--ok)':'var(--err)'};margin-top:2px">${t.pass?'✓':'✗'} ${esc(t.name)}${t.error?' — '+esc(t.error):''}</div>`).join('')}
            </div>
          </div>`;
          ...
          failed++;
          resultEl.innerHTML += `<div class="cr-result-item fail"><span>❌</span><div><div style="font-weight:600;font-size:12px">${esc(req.name)}</div><div style="font-size:11px;color:var(--err)">${esc(err.message)}</div></div></div>`;
          if(stop) { notify('Stopped on error','warn'); return; }
        }
        done++;
...
function createColl() {
  ...
  S.collections.push({id:uid(), name, desc:document.getElementById('nc-desc').value, requests:[], variables:{}, created:Date.now()});
  save(); renderCollections(); closeModal(); notify('Collection created!','success');
}
function delColl(e, id) {
  e.stopPropagation();
  if(!confirm('Delete this collection?')) return;
  S.collections=S.collections.filter(c=>c.id!==id); save(); renderCollections();
}
function addToColl(e, id) {
  e.stopPropagation(); saveTabUI();
  const tab=getActiveTab(), coll=S.collections.find(c=>c.id===id);
  if(!coll||!tab) return;
  const name=prompt('Request name:',tab.name||'New Request'); if(!name) return;
  if(!coll.requests) coll.requests=[];
  coll.requests.push({id:uid(), name, method:tab.method||'GET', url:tab.url||'',
    headers:tab.headers||[], params:tab.params||[], rawBody:tab.rawBody||'',
    bodyType:tab.bodyType||'none', rawFmt:tab.rawFmt||'json',
    authType:tab.authType||'none', authData:tab.authData||{},
    preScript:tab.preScript||'', testScript:tab.testScript||''});
...
function doSave() {
  ...
  const coll=S.collections.find(c=>c.id===id), tab=getActiveTab();
  if(!coll||!name) return;
  if(!coll.requests) coll.requests=[];
  coll.requests.push({id:uid(), name, method:tab?.method||'GET', url:tab?.url||'',
    headers:tab?.headers||[], params:tab?.params||[], rawBody:tab?.rawBody||'',
  ...
  save(); renderCollections(); renderTabs(); closeModal(); notify('Saved!','success');
}
function loadCollReq(cid, rid) {
  const coll=S.collections.find(c=>c.id===cid);
  let req = coll?.requests?.find(r=>r.id===rid);
  if(!req) { for(const item of coll?.requests||[]) { if(item._isFolder) { req=item.requests?.find(r=>r.id===rid); if(req) break; } } }
  if(!req) return;
  newTab({...req});
}
function dupReq(e, cid, rid) {
  e.stopPropagation();
  const coll=S.collections.find(c=>c.id===cid);
  const req=coll?.requests?.find(r=>r.id===rid);
  if(!req||!coll) return;
  coll.requests.push({...req, id:uid(), name:req.name+' (copy)'});
  save(); renderCollections(); notify('Duplicated!','success');
}
function delReq(e, cid, rid) {
  e.stopPropagation();
  const coll=S.collections.find(c=>c.id===cid); if(!coll) return;
  coll.requests=coll.requests.filter(r=>r.id!==rid);
  save(); renderCollections();
}
function exportColl(e, id) {
  ...
  const coll=S.collections.find(c=>c.id===id); if(!coll) return;
  const data = {
    info:{ name:coll.name, description:coll.desc||'', schema:'https://schema.getpostman.com/json/collection/v2.1.0/collection.json' },
    variable: Object.entries(coll.variables||{}).map(([k,v])=>({key:k,value:v})),
    item: (coll.requests||[]).map(r => r._isFolder
      ? { name:r.name, item:(r.requests||[]).map(mapReqToPostman) }
      : mapReqToPostman(r))
  };
  dl(JSON.stringify(data,null,2), coll.name.replace(/\s+/g,'_')+'.postman_collection.json');
  notify('Exported!','success');
}
function mapReqToPostman(r) {
  return {
    name:r.name, request:{
      method:r.method||'GET',
...
function exportAllColls() {
  dl(JSON.stringify(S.collections,null,2), 'postmanweb_collections.json');
  notify('All collections exported!','success');
}
...
function renderEnvs() {
  ...
  if(!S.envs.length) { list.innerHTML='<div class="empty-state"><div class="ei">🌍</div><p>No environments.<br>Create one to use variables.</p></div>'; return; }
  list.innerHTML = S.envs.map(e =>
    `<div class="env-row${e.id===S.activeEnv?' active-env':''}" onclick="setEnv('${e.id}')">
       <div class="env-dot${e.id===S.activeEnv?' on':''}"></div>
       <span class="env-nm">${esc(e.name)}</span>
       <button class="btn-s" onclick="editEnv(event,'${e.id}')">Edit</button>
       <button class="btn-s" onclick="exportEnv(event,'${e.id}')">⬇</button>
       <button class="btn-s" onclick="delEnv(event,'${e.id}')">🗑</button>
     </div>`
  ).join('');
  refreshEnvQuick();
}
function refreshEnvQuick() {
  const sel=document.getElementById('env-quick'); if(!sel) return;
  sel.innerHTML='<option value="">No Environment</option>'+S.envs.map(e=>`<option value="${e.id}"${e.id===S.activeEnv?' selected':''}>${esc(e.name)}</option>`).join('');
}
function quickEnvSwitch(id) { S.activeEnv=id||null; save(); renderEnvs(); notify(id?`Env: ${S.envs.find(e=>e.id===id)?.name}`:'No environment active','info'); }
function setEnv(id) { S.activeEnv=S.activeEnv===id?null:id; save(); renderEnvs(); const e=S.envs.find(e=>e.id===S.activeEnv); notify(S.activeEnv?`Env: ${e?.name}`:'No environment active','info'); }
function openNewEnv() {
  openModal(`<div class="modal-bg"><div class="modal sm">
...
function createEnv() {
  const name=document.getElementById('ne-name').value.trim(); if(!name) return;
  const env={id:uid(), name, variables:{}};
  S.envs.push(env); save(); renderEnvs(); closeModal();
  editEnv(null, env.id);
}
function editEnv(e, id) {
  if(e) e.stopPropagation();
  const env=S.envs.find(x=>x.id===id); if(!env) return;
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      <div id="ev-list">
        ${vars.map(([k,v])=>`<div class="ev-row">
          <input placeholder="Variable" value="${esc(k)}">
          <input placeholder="Value" value="${esc(v)}">
          <button class="ev-del" onclick="this.parentElement.remove()">✕</button>
        </div>`).join('')}
      </div>
...
function saveEnv(id) {
  ...
  save(); renderEnvs(); closeModal(); notify('Environment saved!','success');
}
function delEnv(e, id) {
  e.stopPropagation(); if(!confirm('Delete this environment?')) return;
  S.envs=S.envs.filter(x=>x.id!==id);
  if(S.activeEnv===id) S.activeEnv=null;
  save(); renderEnvs();
}
function exportEnv(e, id) {
  e.stopPropagation();
  const env=S.envs.find(x=>x.id===id); if(!env) return;
  const data={ id:env.id, name:env.name, values:Object.entries(env.variables||{}).map(([k,v])=>({key:k,value:v,enabled:true})) };
  dl(JSON.stringify(data,null,2), env.name.replace(/\s+/g,'_')+'.postman_environment.json');
}
...
function saveGlobals() {
  ...
  save(); closeModal(); notify('Globals saved!','success');
}
...
function openCodegen() {
  ...
  openModal(`<div class="modal-bg"><div class="modal xl">
    ...
    <div class="mb">
      <div class="lang-tabs">
        ${['cURL','JavaScript (Fetch)','JavaScript (Axios)','Python (requests)','Java (OkHttp)','C# (HttpClient)','Go (net/http)','PHP (Guzzle)','Ruby (Net::HTTP)','Swift (URLSession)','Kotlin (OkHttp)','Rust (reqwest)','Node.js (http)','PowerShell']
          .map(l=>`<button class="lang-tab${l==='cURL'?' active':''}" onclick="switchLang('${esc(l)}',this)">${l}</button>`).join('')}
      </div>
      <textarea id="cg-out" readonly spellcheck="false"></textarea>
...
function genCode(lang) {
  ...
  const authH   = (() => {
    ...
    else if(type==='basic') { const u=document.getElementById('a-user')?.value||'',p=document.getElementById('a-pass')?.value||''; h['Authorization']='Basic '+btoa(u+':'+p); }
    else if(type==='apikey'&&document.getElementById('a-key-in')?.value==='header') { const k=document.getElementById('a-key')?.value,v=document.getElementById('a-key-val')?.value; if(k&&v) h[k]=v; }
    else if(type==='oauth2') { const t=document.getElementById('a-o2')?.value; if(t) h['Authorization']=(document.getElementById('a-o2p')?.value||'Bearer')+' '+t; }
    return h;
  })();
  ...
  const hCurl  = Object.entries(allH).map(([k,v])=>`\\\n  --header '${k}: ${v}'`).join(' ');
  const hFetch = JSON.stringify(allH,null,2);
  const hPy    = JSON.stringify(allH,null,4);

  const codes = {
    'cURL': ()=>{
      let c=`curl --location --request ${method} '${url}'`;
      if(Object.keys(allH).length) c+=` ${hCurl}`;
      if(hasBody) c+=` \\\n  --data-raw '${rawBody.replace(/'/g,"'\\''")}'`;
      return c;
    },
    'JavaScript (Fetch)': ()=>`const myHeaders = new Headers(${hFetch});\n\nconst requestOptions = {\n  method: "${method}",\n  headers: myHeaders,\n  ${hasBody?`body: ${JSON.stringify(rawBody)},\n  `:''}redirect: "follow"\n};\n\nfetch("${url}", requestOptions)\n  .then(response => response.json())\n  .then(result => console.log(result))\n  .catch(error => console.error("Error:", error));`,
    'JavaScript (Axios)': ()=>{
    ...
    'C# (HttpClient)': ()=>{
      const hStr=Object.entries(allH).map(([k,v])=>`client.DefaultRequestHeaders.Add("${k}", "${v}");`).join('\n');
    ...
    'Go (net/http)': ()=>{
      const body2=hasBody?`strings.NewReader(${JSON.stringify(rawBody)})`:'nil';
    ...
    'Swift (URLSession)': ()=>{
      const hStr=Object.entries(allH).map(([k,v])=>`request.setValue("${v}", forHTTPHeaderField: "${k}")`).join('\n');
  ...
  if(out) out.value = (codes[lang]||codes['cURL'])();
}
...
function openWS() {
  openModal(`<div class="modal-bg"><div class="modal lg">
    <div class="mh"><span class="mh-title">🔌 WebSocket Client</span><button class="m-close" onclick="closeModal();wsDisconnect()">✕</button></div>
    <div class="mb">
      <div style="display:flex;gap:8px;margin-bottom:10px">
        <input id="ws-url" type="text" placeholder="wss://echo.websocket.org" style="flex:1">
        <button class="btn primary" id="ws-btn" onclick="wsToggle()">Connect</button>
      </div>
      <div style="display:flex;gap:8px;margin-bottom:10px">
        <input id="ws-msg" type="text" placeholder='{"type":"ping"}' style="flex:1">
        <button class="btn secondary" onclick="wsSend()">Send</button>
      </div>
      <div id="ws-msgs" style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);min-height:200px;max-height:350px;overflow-y:auto;padding:4px"></div>
    </div>
    <div class="mf"><button class="btn secondary" onclick="closeModal();wsDisconnect()">Close</button></div>
  </div></div>`);
}
function wsToggle() {
  if(_wsConn&&_wsConn.readyState===WebSocket.OPEN) wsDisconnect();
  else wsConnect();
}
function wsConnect() {
  const url=document.getElementById('ws-url')?.value?.trim();
  if(!url) { notify('Enter WebSocket URL','error'); return; }
  try {
    _wsConn = new WebSocket(url);
    wsLog(`• Connecting to ${url}...`, 'sys');
    _wsConn.onopen    = () => { wsLog('✅ Connected!','sys'); const b=document.getElementById('ws-btn'); if(b){b.textContent='Disconnect';b.style.background='var(--err)';} };
    _wsConn.onmessage = e => wsLog('← '+e.data,'recv');
    _wsConn.onerror   = () => wsLog('❌ Connection error','sys');
    _wsConn.onclose   = () => { wsLog('• Connection closed','sys'); const b=document.getElementById('ws-btn'); if(b){b.textContent='Connect';b.style.background='';} };
...
function wsSend() {
  ...
  _wsConn.send(msg); wsLog('→ '+msg,'sent');
  document.getElementById('ws-msg').value='';
}
function wsLog(msg, cls) {
  const d=document.getElementById('ws-msgs'); if(!d) return;
  const div=document.createElement('div'); div.className='ws-line '+cls; div.textContent=msg;
...
function openGRPC() {
  openModal(`<div class="modal-bg"><div class="modal lg">
    <div class="mh"><span class="mh-title">gRPC Client</span><button class="m-close" onclick="closeModal()">✕</button></div>
    <div class="mb">
      <div class="fg"><label>SERVER URL</label><input id="grpc-url" placeholder="https://grpc.example.com"></div>
      <div class="fg"><label>SERVICE METHOD</label><input id="grpc-method" placeholder="package.ServiceName/MethodName"></div>
      <div class="fg"><label>REQUEST BODY (JSON)</label><textarea id="grpc-body" rows="6" class="code-area" placeholder='{"key":"value"}'></textarea></div>
      <div class="fg"><label>METADATA (JSON)</label><textarea id="grpc-meta" rows="3" class="code-area" placeholder='{"authorization":"Bearer token"}'></textarea></div>
      <div id="grpc-resp" style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);min-height:80px;max-height:200px;overflow-y:auto;padding:10px;font-family:var(--mono);font-size:12px;color:var(--text3);margin-top:8px">
        gRPC response will appear here...
      </div>
    </div>
    <div class="mf">
      <button class="btn secondary" onclick="closeModal()">Close</button>
      <button class="btn primary" onclick="invokeGRPC()">Invoke</button>
    </div>
  </div></div>`);
}
async function invokeGRPC() {
  const url    = document.getElementById('grpc-url')?.value?.trim();
  const method = document.getElementById('grpc-method')?.value?.trim();
  const body   = document.getElementById('grpc-body')?.value?.trim();
  const meta   = document.getElementById('grpc-meta')?.value?.trim();
  const respEl = document.getElementById('grpc-resp');
  if(!url||!method) { notify('URL and method required','error'); return; }
  respEl.innerHTML = '<span style="color:var(--text3)">⏳ Invoking gRPC method...</span>';
  // Browser gRPC via grpc-web over HTTP proxy
  const proxyUrl = S.settings.corsEnabled
    ? S.settings.proxyUrl+encodeURIComponent(url+'/'+method)
    : url+'/'+method;
  let headers = {'Content-Type':'application/grpc-web+json','x-grpc-web':'1'};
  if(meta) { try{ Object.assign(headers,JSON.parse(meta)); }catch{} }
  try {
    const r = await fetch(proxyUrl, { method:'POST', headers, body:body||'{}' });
    const txt = await r.text();
    let parsed; try{ parsed=JSON.parse(txt); }catch{ parsed=txt; }
    respEl.innerHTML = `<span style="color:var(--ok)">Status: ${r.status} ${r.statusText}</span>\n\n${esc(typeof parsed==='string'?parsed:JSON.stringify(parsed,null,2))}`;
    notify('gRPC invoked: '+r.status, r.ok?'success':'error');
  } catch(e) {
    respEl.innerHTML=`<span style="color:var(--err)">${esc(e.message)}</span>`;
    notify('gRPC error: '+e.message,'error');
  }
}
...
function openMockServer() {
  openModal(`<div class="modal-bg"><div class="modal lg">
    <div class="mh"><span class="mh-title">🎭 Mock Server</span><button class="m-close" onclick="closeModal()">✕</button></div>
    <div class="mb">
      <p style="font-size:12px;color:var(--text3);margin-bottom:12px">
        Create mock rules to intercept requests. Enable "Use Mock Response" in Request Settings or the per-request toggle.
      </p>
      <div id="mock-rules">
        ${S.mocks.length ? S.mocks.map((m,i)=>mockRuleHtml(m,i)).join('') : '<p style="color:var(--text3);font-size:12px">No mock rules yet.</p>'}
      </div>
      <button class="add-row-btn" style="margin-top:10px" onclick="addMockRule()">+ Add Mock Rule</button>
    </div>
    <div class="mf">
      <button class="btn secondary" onclick="closeModal()">Close</button>
      <button class="btn primary" onclick="saveMockRules()">Save Rules</button>
    </div>
  </div></div>`);
}
function mockRuleHtml(m, i) {
  return `<div class="mock-rule" id="mock-rule-${i}">
    <div class="mock-rule-hdr" style="margin-bottom:8px">
      <select style="width:90px" id="mr-method-${i}">
        ${['*','GET','POST','PUT','PATCH','DELETE','HEAD','OPTIONS'].map(x=>`<option${x===m.method?' selected':''}>${x}</option>`).join('')}
      </select>
      <input type="text" id="mr-path-${i}"   value="${esc(m.path||'')}" placeholder="/api/users" style="flex:1">
      <select id="mr-status-${i}" style="width:80px">
        ${[200,201,204,400,401,403,404,422,500,502,503].map(s=>`<option${s===m.statusCode?' selected':''}>${s}</option>`).join('')}
      </select>
      <input type="number" id="mr-delay-${i}" value="${m.delay||0}" placeholder="delay ms" style="width:70px">
      <label class="toggle"><input type="checkbox" id="mr-en-${i}"${m.enabled!==false?' checked':''}><span class="t-slider"></span></label>
      <button class="icon-btn del" onclick="removeMockRule(${i})">🗑</button>
    </div>
    <div class="fg"><label>RESPONSE BODY</label>
      <textarea id="mr-body-${i}" class="code-area" rows="4">${esc(m.body||'{}')}</textarea>
    </div>
    <div class="fg"><label>CONTENT TYPE</label>
      <input type="text" id="mr-ct-${i}" value="${esc(m.contentType||'application/json')}">
    </div>
  </div>`;
}
function addMockRule() {
  S.mocks.push({id:uid(), method:'GET', path:'', statusCode:200, body:'{}', contentType:'application/json', delay:0, enabled:true});
  document.getElementById('mock-rules').innerHTML = S.mocks.map((m,i)=>mockRuleHtml(m,i)).join('');
}
function removeMockRule(i) { S.mocks.splice(i,1); save(); openMockServer(); }
function saveMockRules() {
  S.mocks = S.mocks.map((_,i)=>({
    id:   S.mocks[i].id||uid(),
    method:     document.getElementById('mr-method-'+i)?.value||'GET',
    path:       document.getElementById('mr-path-'+i)?.value||'',
    statusCode: parseInt(document.getElementById('mr-status-'+i)?.value)||200,
    body:       document.getElementById('mr-body-'+i)?.value||'{}',
    contentType:document.getElementById('mr-ct-'+i)?.value||'application/json',
    delay:      parseInt(document.getElementById('mr-delay-'+i)?.value)||0,
    enabled:    document.getElementById('mr-en-'+i)?.checked!==false,
  }));
  save(); closeModal(); notify('Mock rules saved!','success');
}
...
// ============================================================
function openImport() {
  openModal(`<div class="modal-bg"><div class="modal md">
    <div class="mh"><span class="mh-title">📥 Import</span><button class="m-close" onclick="closeModal()">✕</button></div>
    <div class="mb">
      <div class="fg"><label>PASTE JSON (Postman Collection v2.1, Environment, OpenAPI) OR cURL COMMAND</label>
        <textarea id="imp-txt" rows="10" style="width:100%;background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:10px;color:var(--text1);font-family:var(--mono);font-size:12px;resize:vertical" placeholder="Paste Postman Collection JSON, Environment JSON, OpenAPI JSON/YAML, or curl command..."></textarea>
      </div>
      <div class="fg"><label>OR UPLOAD FILE (.json, .yaml, .yml)</label>
        <input type="file" id="imp-file" accept=".json,.yaml,.yml" onchange="loadImpFile(this)">
      </div>
    </div>
...
function doImport() {
  ...
  try {
    ...
    if(data.info&&data.item) {
      ...
      const flat=(items,target)=>items?.forEach(item=>{
        if(item.item) {
          // folder
          const folder={id:uid(), name:item.name, _isFolder:true, requests:[]};
          flat(item.item, folder.requests);
          target.push(folder);
          ...
          target.push({
            ...
            bodyType:item.request.body?.mode==='raw'?'raw':item.request.body?.mode||'none',
            rawFmt:'json',
            authType:item.request.auth?.type||'none', authData:{},
      ...
      flat(data.item, coll.requests);
      S.collections.push(coll); save(); renderCollections(); closeModal();
      notify(`✅ Imported "${coll.name}" — ${coll.requests.length} items`,'success'); return;
    }
    // Postman Environment
    if(data.values&&(data.name||data.id)) {
      const env={id:uid(), name:data.name||'Imported Env', variables:{}};
      (data.values||[]).forEach(v=>{ env.variables[v.key]=v.value; });
      S.envs.push(env); save(); renderEnvs(); closeModal();
      notify(`✅ Imported environment "${env.name}"`,'success'); return;
    }
    // PostmanWeb backup
...
function importOpenAPI(spec) {
  ...
  Object.entries(paths).forEach(([path,pathItem])=>{
    ['get','post','put','patch','delete','head','options'].forEach(m=>{
      if(!pathItem[m]) return;
      const op=pathItem[m];
      const headers=[]; const params=[];
      (op.parameters||[]).forEach(p=>{
        if(p.in==='header') headers.push({id:uid(),on:true,k:p.name,v:p.example||'',desc:p.description||''});
        else if(p.in==='query') params.push({id:uid(),on:true,k:p.name,v:p.example||'',desc:p.description||''});
      });
      ...
      coll.requests.push({
        id:uid(), name:op.summary||op.operationId||(m.toUpperCase()+' '+path),
        method:m.toUpperCase(), url:base+path, headers, params, rawBody, bodyType, rawFmt:'json',
        authType:'none', authData:{}, preScript:'', testScript:'',
      });
    });
  });
  S.collections.push(coll); save(); renderCollections();
  notify(`✅ OpenAPI imported — ${coll.requests.length} endpoints`,'success');
}
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">
        <input id="ck-d" placeholder="Domain (api.example.com)" style="flex:1;min-width:140px">
        <input id="ck-n" placeholder="Name" style="width:120px">
...
function renderCKList() {
  const d=Object.keys(S.cookies);
  if(!d.length) return '<div class="empty-state"><div class="ei">🍪</div><p>No cookies stored.</p></div>';
  return d.map(domain=>
    `<div class="ck-domain"><div class="ck-domain-nm">${esc(domain)}</div>`+
    Object.entries(S.cookies[domain]).map(([k,v])=>
      `<div class="ck-row">
         <span class="ck-name">${esc(k)}</span>
         <span class="ck-val">${esc(v)}</span>
         <button onclick="delCK('${esc(domain)}','${esc(k)}')" style="color:var(--err);background:none;border:none;cursor:pointer;margin-left:auto;font-size:12px">✕</button>
       </div>`
    ).join('')+'</div>'
  ).join('');
}
function addCK() {
  const d=document.getElementById('ck-d').value.trim(), n=document.getElementById('ck-n').value.trim(), v=document.getElementById('ck-v').value;
  if(!d||!n) { notify('Domain and name required','error'); return; }
  if(!S.cookies[d]) S.cookies[d]={};
  S.cookies[d][n]=v; save();
  ...
  openModal(`<div class="modal-bg"><div class="modal lg">
    ...
    <div class="mb">
      <div class="s-sec">
        ...
        <div class="s-row">
          <div><div class="s-label">Enable CORS Proxy</div><div class="s-desc">Route all requests through Cloudflare Worker to bypass browser CORS</div></div>
        ...
        <div class="fg" style="margin-top:10px"><label>PROXY URL</label>
          <input id="set-proxy" value="${esc(s.proxyUrl||'https://square-credit-8186.donthulanithish53.workers.dev/?url=')}">
        </div>
      ...
      <div class="s-sec">
        <div class="s-sec-title">THEME</div>
        <div class="s-row">
          <div><div class="s-label">Dark Mode</div><div class="s-desc">Toggle between dark and light themes</div></div>
          <label class="toggle"><input type="checkbox" id="set-dark"${s.theme!=='light'?' checked':''} onchange="toggleThemeFromSettings(this)"><span class="t-slider"></span></label>
        </div>
      </div>
      <div class="s-sec">
      ...
      <div class="s-sec">
        ...
        <div style="font-size:11px;color:var(--text2);line-height:2.2;font-family:var(--mono)">
          ...
          <b>Ctrl+S</b> Save  &nbsp; <b>Ctrl+\\</b> Toggle Sidebar  &nbsp; <b>Esc</b> Cancel Request
        </div>
      </div>
      <div class="s-sec">
        <div class="s-sec-title">ABOUT</div>
        <p style="font-size:12px;color:var(--text3);line-height:1.8">
          PostmanWeb v4 — Full API Testing Platform in your browser.<br>
          All data stored locally in localStorage. No server. No login required.<br>
          Worker: <span style="color:var(--accent)">square-credit-8186.donthulanithish53.workers.dev</span>
        </p>
      </div>
    </div>
...
async function testProxy() {
  ...
  try {
    ...
    else{res.textContent=`⚠ Worker: ${r.status}`;res.style.color='var(--warn)';}
...
function saveSettings() {
  ...
  save(); refreshCORSBtn(); closeModal(); notify('Settings saved!','success');
}
function exportAll() {
  dl(JSON.stringify({collections:S.collections,envs:S.envs,globals:S.globals,history:S.history,settings:S.settings,mocks:S.mocks},null,2),'postmanweb_backup.json');
  ...
  inp.onchange=e=>{
    ...
    r.onload=ev=>{
      try {
        ...
        if(d.mocks)       S.mocks=d.mocks;
        save(); renderAll(); notify('Backup imported!','success');
...
function renderWorkspaces() {
  const sel=document.getElementById('ws-sel'); if(!sel) return;
  sel.innerHTML=S.workspaces.map(w=>`<option value="${w.id}"${w.id===S.activeWS?' selected':''}>${esc(w.name)}</option>`).join('');
}
function switchWorkspace(id) { S.activeWS=id; save(); notify('Switched workspace','info'); }
function openNewWorkspace() {
  const name=prompt('Workspace name:'); if(!name) return;
  const ws={id:uid(), name}; S.workspaces.push(ws); S.activeWS=ws.id;
  save(); renderWorkspaces(); notify('Workspace created!','success');
}
...
function applyTheme() {
  ...
  if(btn) btn.textContent = S.settings.theme==='light' ? '🌙' : '☀️';
}
...
function initResize() {
  ...
  handle.addEventListener('mousedown',e=>{
    ...
    sh=document.getElementById('req-area').offsetHeight;
    document.body.style.userSelect='none';
    document.body.style.cursor='ns-resize';
  });
  document.addEventListener('mousemove',e=>{
    if(!dragging) return;
    const nh=Math.max(80, Math.min(wrap.offsetHeight-80, sh+(e.clientY-sy)));
...
// ============================================================
function renderAll() {
  renderTabs(); renderCollections(); renderHistory(); renderEnvs(); renderWorkspaces();
}
...
function init() {
  ...
  // Auto-update tab name from URL input
  document.getElementById('url-in').addEventListener('input', e => {
    ...
    if(tab&&e.target.value){
      ...
      tab.name=clean.slice(0,40)||'New Request';
      renderTabs();
      // Auto-detect path variables
